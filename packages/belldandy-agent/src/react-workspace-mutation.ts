...
> = {
  ...
  residual_identifier_requires_removal: "The runner-computed post-write residual scan found forbidden identifiers still present in required paths. Treat the per-path counts and first line numbers in the residual scan as the authoritative remaining-work list. Use exactly ONE apply_patch call now that removes or migrates EVERY listed occurrence in this single patch, so the next scan reports zero; a partial removal is not completion. Enumerate each occurrence as its own hunk: prefer exact evidence lines as hunk anchors, and when the same line appears multiple times use unique surrounding context so each hunk lands on the intended occurrence.",
};
