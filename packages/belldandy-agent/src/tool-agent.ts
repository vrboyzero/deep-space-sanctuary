    ...
    const logError = (msg: string, data?: unknown) => {
      ...
      while (true) {
        ...
        if (!toolCalls || toolCalls.length === 0) {
          ...
          const priorPatchAdjacentDuplicateClosingDelimiter = workspaceMutationObjectiveReviewCall
            && hasPriorPatchAdjacentDuplicateClosingDelimiterCurrentSource(
              mutationRecoverySourceMessages,
              input.text,
              successfulWorkspaceMutationPatchInputs,
            );
          const noopSerializedFalseRemovalBranch = workspaceMutationObjectiveReviewCall
            && hasNoopSerializedFalseRemovalBranchCurrentSource(
              mutationRecoverySourceMessages,
              input.text,
              successfulWorkspaceMutationPatchInputs,
          ...
          if (workspaceMutationObjectiveReviewCall && input.structuredOutput) {
            ...
            if (!objectiveValidation.ok) {
              ...
              if (canCorrectDeterministicObjectiveFailure) {
                ...
                recentToolCallTraces.length = 0;
                lastSuccessfulToolResult = undefined;
                logWarn("[workspace-mutation] invalid objective output accompanied deterministic current-source failure; scheduling one input correction", {
                  requiredPathCount: requiredChangedPaths.length,
                  conversationId: input.conversationId,
                  agentId: resolvedAgentId,
                });
        ...
        if (workspaceMutationRecoveryCall || workspaceMutationObjectiveReviewCall) {
          ...
          const rebuiltSerializedFalseToolCall = workspaceMutationObjectiveInputCorrectionCall
            ...
            : undefined;
          const rebuiltSerializedFalseInitialNoOpRecoveryToolCall = workspaceMutationRecoveryCall
            ? rebuildSerializedFalseInitialNoOpToolCall({
              toolCall: constrainedMutationToolCall,
              messages: mutationRecoverySourceMessages,
              taskText: input.text,
          ...
          const rebuiltSerializedFalseNarrowArPrefixToolCall = workspaceMutationObjectiveReviewCall
            ...
            : undefined;
          const rebuiltSerializedFalseDroppedFallbackToolCall = workspaceMutationObjectiveReviewCall
            ? rebuildSerializedFalseDroppedFallbackToolCall({
                toolCall: constrainedMutationToolCall,
                messages: mutationRecoverySourceMessages,
                taskText: input.text,
          ...
          const rebuiltSerializedFalseSiblingDoubleElseToolCall = workspaceMutationObjectiveReviewCall
            ...
            : undefined;
          const rebuiltSerializedFalseNestedUnreachableToolCall = workspaceMutationObjectiveReviewCall
            && !workspaceMutationObjectiveInputCorrectionCall
            ? rebuildSerializedFalseNestedUnreachableToolCall({
              toolCall: constrainedMutationToolCall,
              messages: mutationRecoverySourceMessages,
          ...
          if (rebuiltSerializedFalseNarrowArPrefixToolCall) {
            logWarn("[workspace-mutation] rebuilt trusted narrow ar-prefix correction as baseline serialized-false condition", {
              requiredPathCount: workspaceMutationCallRequiredPaths.length,
              conversationId: input.conversationId,
              agentId: resolvedAgentId,
            });
          }
          if (rebuiltSerializedFalseDroppedFallbackToolCall) {
            logWarn("[workspace-mutation] rebuilt trusted dropped-fallback correction as a complete serialized-false branch", {
              requiredPathCount: workspaceMutationCallRequiredPaths.length,
              conversationId: input.conversationId,
              agentId: resolvedAgentId,
            });
          ...
          if (rebuiltSerializedFalseNestedUnreachableToolCall) {
            logWarn("[workspace-mutation] rebuilt trusted nested unreachable-false correction as baseline serialized-false condition", {
              requiredPathCount: workspaceMutationCallRequiredPaths.length,
              conversationId: input.conversationId,
              agentId: resolvedAgentId,
            });
          ...
          if (workspaceMutationObjectiveReviewCall
            ...
            yield* emitWorkspaceMutationFailure(
              nonAtomicSerializedFalseRemovalCorrection
                ? "the post-write no-op removal correction changed lines outside its atomic condition-and-removeAttribute repair."
