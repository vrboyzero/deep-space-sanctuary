...
describe("ToolEnabledAgent required workspace mutation", () => {
  ...
  it("rejects a correction that re-adds the prior broad line inside a small aria block", async () => {
    const requiredChangedPaths = ["src/diff/props.js"];
    const originalCondition = "\t\t} else if (value != NULL && value !== false) {";
    const broadCondition = "\t\t} else if (value != NULL) {";
    const correctedCondition = "\t\t} else if (value != NULL && (value !== false || name[4] == '-')) {";
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      ...
      if (instruction.includes("Post-mutation objective review phase")) {
        return response(modelToolCall("rewrite-retained-broad-line", "apply_patch", {
          input: retainedExpandedPatch,
        }, 300, 60));
      }
      return response(modelToolCall("patch-broad", "apply_patch", {
        input: broadPatch,
    ...
    const execute = vi.fn(async (request: {
      ...
      const content = executedPatches.includes(correctedPatch) ? correctedSource : broadSource;
      return {
        id: request.id,
        name: request.name,
        success: true,
        output: JSON.stringify({ path: request.arguments?.path, truncated: false, content }),
        durationMs: 1,
      };
    });
    const agent = createAgent({
    ...
    const items = await collect(agent.run({
      conversationId: "conv-required-mutation-retained-broad-line-correction",
      text: "Restore false aria-* attribute serialization with the smallest change while preserving the public behavior of other attributes.",
      automationProfile: "bare",
      meta: {
        _agentLaunchSpec: {
    ...
    expect(executedPatches).toEqual([broadPatch, correctedPatch]);
    expect(executedPatches).not.toContain(retainedExpandedPatch);
    expect(items).toContainEqual({ type: "final", text: "corrected and verified" });
    expect(items.at(-1)).toEqual({ type: "status", status: "done" });
  });
  ...
  it("does not correct a broadened mutation by reverting the requested positive case", async () => {
    ...
    const revertedSource = broadSource.replace(
      "} else if (value != NULL) {",
      "} else if (value != NULL && value !== false) {",
    );
    const correctedSource = broadSource.replace(
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      ...
      if (requests.length === 3) {
        ...
        return response(modelToolCall(
          ...
          "apply_patch",
          {
            input: reviewInstruction.includes("positive witness")
              ? correctionPatch
              : revertingPatch,
          },
          300,
    ...
    const execute = vi.fn(async (request: {
      ...
      if (request.name === "apply_patch") {
        mutationCount++;
        lastPatch = String(request.arguments?.input ?? "");
        return {
          id: request.id,
          name: request.name,
          success: true,
      ...
      const content = mutationCount === 1
        ? broadSource
        : lastPatch === correctionPatch
          ? correctedSource
          : revertedSource;
      return {
        id: request.id,
    ...
    const items = await collect(agent.run({
      conversationId: "conv-required-mutation-subset-positive-witness",
      text: "Restore false aria-* attribute serialization with the smallest change while preserving the public behavior of other attributes.",
      automationProfile: "bare",
      meta: {
        _agentLaunchSpec: {
    ...
    expect(requests).toHaveLength(5);
    expect(requests[2]?.messages[0]?.content).toContain("positive witness");
    expect(requests[2]?.messages[0]?.content).toContain("negative witness");
    expect(lastPatch).toBe(correctionPatch);
    expect(items).toContainEqual({ type: "final", text: "corrected and verified" });
    expect(items.at(-1)).toEqual({ type: "status", status: "done" });
  });
