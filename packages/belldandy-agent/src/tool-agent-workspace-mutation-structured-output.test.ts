...
describe("ToolEnabledAgent post-mutation structured output", () => {
  ...
  it("accepts only grouping parentheses for a false aria/data precedence correction", async () => {
    const requiredPath = "src/diff/props.js";
    const originalCondition = "\t\t} else if (value != NULL && value !== false) {";
    const setAttributeLine = "\t\t\tdom.setAttribute(name, name == 'popover' && value == true ? '' : value);";
    ...
    const task = "Fix the frozen browser-facing regression in the real web project. Preserve false values for aria-* and data-* attributes by serializing them, remove ordinary attributes with false values, and remove every attribute with null or undefined values. Make the smallest change in src/diff/props.js and pass the supplied deterministic checks.";
    const successfulSummary = '{"summary":"Verified false aria/data serialization and nullish removal."}';
    const localRejection = "operator precedence lets the data-* predicate bypass `value === false`";
    const requests: Array<Record<string, any>> = [];
    const executedPatches: string[] = [];
    vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      const body = JSON.parse(String(init?.body ?? "{}")) as Record<string, any>;
      ...
      if (instruction.includes("Post-mutation objective correction input retry phase")) {
        return jsonResponse(modelToolCall("group-precedence", "apply_patch", {
          input: instruction.includes(localRejection) ? groupingCorrection : broadCorrection,
        }, 300, 60));
      }
      if (instruction.includes("Post-mutation final objective review phase")) {
        return jsonResponse({
      ...
      if (instruction.includes("Post-mutation objective review phase")) {
        return jsonResponse(modelToolCall("broad-precedence", "apply_patch", {
          input: broadCorrection,
        }, 300, 60));
      }
      return jsonResponse(modelToolCall("patch-precedence", "apply_patch", {
        input: initialPatch,
      }, 300, 60));
    });
    const execute = vi.fn(async (request: {
      ...
      return {
        ...
        output: JSON.stringify({
          path: request.arguments?.path,
          truncated: false,
          content: executedPatches.includes(groupingCorrection)
            ? postCorrectionSource
            : postInitialSource,
        }),
        durationMs: 1,
    ...
    const items = await collect(agent.run({
      conversationId: "conv-required-mutation-false-precedence-grouping",
      text: task,
      automationProfile: "bare",
      meta: {
        _agentLaunchSpec: {
    ...
    expect(requests).toHaveLength(6);
    expect(requests[3]?.messages[0]?.content).toContain(localRejection);
    expect(executedPatches).toEqual([initialPatch, groupingCorrection]);
    expect(executedPatches).not.toContain(broadCorrection);
    expect(items).toContainEqual({ type: "final", text: successfulSummary });
    expect(items.at(-1)).toEqual({ type: "status", status: "done" });
  });

  it("repairs only the aria-only ternary wrapper that makes data false unreachable", async () => {
    const requiredPath = "src/diff/props.js";
    const originalCondition = "\t\t} else if (value != NULL && value !== false) {";
    const setAttributeLine = "\t\t\tdom.setAttribute(name, name == 'popover' && value == true ? '' : value);";
    const ariaPredicate = "(name[0] == 'a' && name[1] == 'r' && name[2] == 'i' && name[3] == 'a' && name[4] == '-')";
    ...
    const nullGuardCorrectionCondition = `\t\t} else if (value != NULL && (name[0] == 'a' && (name[1] == 'r' || name[1] == 'a') ? ${ariaPredicate} || ${dataPredicate} : false)) {`;
    ...
    const initialPatch = [
      ...
      "+\t\t\t// aria-* and data-* have no boolean representation.",
      "+\t\t\t// A `false` value is different from the attribute not",
      "+\t\t\t// being present, so we serialize it as the string false.",
    ...
    const ineffectiveCorrection = [
      "*** Begin Patch",
      `*** Update File: ${requiredPath}`,
      "@@",
      `-${unreachableCondition}`,
      `+${nullGuardCorrectionCondition}`,
      " \t\t\t// aria-* and data-* have no boolean representation.",
      "*** End Patch",
    ].join("\n");
    const reachabilityCorrection = [
      "*** Begin Patch",
      `*** Update File: ${requiredPath}`,
      "@@",
      `-${unreachableCondition}`,
    ...
    const task = "Fix the frozen browser-facing regression in the real web project. Preserve false values for aria-* and data-* attributes by serializing them, remove ordinary attributes with false values, and remove every attribute with null or undefined values. Make the smallest change in src/diff/props.js and pass the supplied deterministic checks.";
    const successfulSummary = '{"summary":"Verified reachable false aria/data serialization and nullish removal."}';
    const localRejection = "the outer aria-only ternary makes the data-* serialized-false predicate unreachable";
    const requests: Array<Record<string, any>> = [];
    const executedPatches: string[] = [];
    vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      const body = JSON.parse(String(init?.body ?? "{}")) as Record<string, any>;
      ...
      if (instruction.includes("Post-mutation objective correction input retry phase")) {
        return jsonResponse(modelToolCall("restore-data-reachability", "apply_patch", {
          input: instruction.includes(localRejection)
            ? reachabilityCorrection
            : ineffectiveCorrection,
        }, 300, 60));
      }
      if (instruction.includes("Post-mutation final objective review phase")) {
        return jsonResponse({
      ...
      if (instruction.includes("Post-mutation objective review phase")) {
        return jsonResponse(modelToolCall("add-null-guard", "apply_patch", {
          input: ineffectiveCorrection,
        }, 300, 60));
      }
      return jsonResponse(modelToolCall("patch-unreachable-data", "apply_patch", {
        input: initialPatch,
      }, 300, 60));
    });
    const execute = vi.fn(async (request: {
      ...
      return {
        ...
        output: JSON.stringify({
          path: request.arguments?.path,
          truncated: false,
          content: executedPatches.includes(reachabilityCorrection)
            ? postCorrectionSource
            : postInitialSource,
        }),
        durationMs: 1,
    ...
    const items = await collect(agent.run({
      conversationId: "conv-required-mutation-unreachable-data-ternary",
      text: task,
      automationProfile: "bare",
      meta: {
        _agentLaunchSpec: {
    ...
    expect(requests).toHaveLength(6);
    expect(requests[3]?.messages[0]?.content).toContain(localRejection);
    expect(executedPatches).toEqual([initialPatch, reachabilityCorrection]);
    expect(executedPatches).not.toContain(ineffectiveCorrection);
    expect(items).toContainEqual({ type: "final", text: successfulSummary });
    expect(items.at(-1)).toEqual({ type: "status", status: "done" });
  });

  it("repairs only the parent false guard that makes nested aria/data serialization unreachable", async () => {
    const requiredPath = "src/diff/props.js";
    ...
    const task = "Fix the frozen browser-facing regression in the real web project. Preserve false values for aria-* and data-* attributes by serializing them, remove ordinary attributes with false values, and remove every attribute with null or undefined values. Make the smallest change in src/diff/props.js and pass the supplied deterministic checks.";
    const successfulSummary = '{"summary":"Verified reachable false aria/data serialization and nullish removal."}';
    const localRejection = "the parent value !== false guard makes the nested aria/data serialized-false branch unreachable";
    const requests: Array<Record<string, any>> = [];
    const executedPatches: string[] = [];
    vi.spyOn(globalThis, "fetch").mockImplementation(async (_url, init) => {
      const body = JSON.parse(String(init?.body ?? "{}")) as Record<string, any>;
      ...
      if (instruction.includes("Post-mutation objective correction input retry phase")) {
        return jsonResponse(modelToolCall("restore-parent-reachability", "apply_patch", {
          input: instruction.includes(localRejection)
            ? parentGuardCorrection
            : repeatedCurrentSource,
        }, 300, 60));
      }
      if (instruction.includes("Post-mutation final objective review phase")) {
        return jsonResponse({
      ...
      if (instruction.includes("Post-mutation objective review phase")) {
        return jsonResponse(modelToolCall("repeat-current-source", "apply_patch", {
          input: repeatedCurrentSource,
        }, 300, 60));
      }
      return jsonResponse(modelToolCall("patch-parent-false-guard", "apply_patch", {
        input: initialPatch,
      }, 300, 60));
    });
    const execute = vi.fn(async (request: {
      ...
      return {
        ...
        output: JSON.stringify({
          path: request.arguments?.path,
          truncated: false,
          content: executedPatches.includes(parentGuardCorrection)
            ? postCorrectionSource
            : postInitialSource,
        }),
        durationMs: 1,
    ...
    const items = await collect(agent.run({
      conversationId: "conv-required-mutation-parent-false-guard",
      text: task,
      automationProfile: "bare",
      meta: {
        _agentLaunchSpec: {
    ...
    expect(requests).toHaveLength(6);
    expect(requests[3]?.messages[0]?.content).toContain(localRejection);
    expect(executedPatches).toEqual([initialPatch, parentGuardCorrection]);
    expect(executedPatches).not.toContain(repeatedCurrentSource);
    expect(items).toContainEqual({ type: "final", text: successfulSummary });
    expect(items.at(-1)).toEqual({ type: "status", status: "done" });
  });
