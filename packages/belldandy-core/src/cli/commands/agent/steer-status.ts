...
export async function steerStatusAgentRunCommand(input: {
  stateDir: string;
  ...
  const result = await invokeGatewayMethod({
    ...
    params: { query },
    requestIdPrefix: "bdd-agent-steer-status",
    clientName: "bdd agent steer-status",
...
export default defineCommand({
  meta: { name: "steer-status", description: "Inspect one Conversation steer command" },
  args: {
    ...
    "command-id": { type: "string", description: "Steer command ID", required: true },
    "state-dir": { type: "string", description: "Override state directory" },
    json: { type: "boolean", description: "Write compact JSON" },
  },
  async run({ args }) {
    const ctx = createCLIContext({ stateDir: args["state-dir"] });
    process.exitCode = await steerStatusAgentRunCommand({
