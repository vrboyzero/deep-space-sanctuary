...
export interface ConfigureClaudeCodeSessionOptions {
  stateDir: string;
...
export interface ConfigureClaudeCodeSessionResult {
  changed: boolean;
  stateDir: string;
  workspaceRoot: string;
  extraWorkspaceRoots: string[];
...
function buildClaudeSessionTarget(options: {
  targetId: string;
  workspaceRoot: string;
  extraWorkspaceRoots?: string[];
  claudeCommand: string;
  ...
  return {
    ...
    entry: {
      ...
      ...(options.gitBashPath
        ? {
            env: {
              CLAUDE_CODE_GIT_BASH_PATH: options.gitBashPath,
            },
          }
        : {}),
    },
    ...
    actions: {
      interactive: {
        ...
        description: "通过 Claude Code CLI 启动持续 bridge session，并由 bridge_session_* 工具持续驱动。",
...
export async function configureClaudeCodeSession(
  options: ConfigureClaudeCodeSessionOptions,
): Promise<ConfigureClaudeCodeSessionResult> {
  ...
  const targetUpsert = upsertById(nextBridge.targets ?? [], buildClaudeSessionTarget({
...
export default defineCommand({
  meta: {
    ...
    description: "Generate a persistent claude_code_session bridge target",
  },
  args: {
    ...
    "target-id": { type: "string", description: "Bridge session target id", default: "claude_code_session" },
  },
  async run({ args }) {
    ...
    const result = await configureClaudeCodeSession({
      ...
      targetId: String(args["target-id"] ?? "claude_code_session"),
    });
    ...
    if (result.changed) {
      ctx.success("Claude session bridge 配置已生成");
    } else {
      ctx.log("Claude session bridge 配置已是最新状态");
    }
