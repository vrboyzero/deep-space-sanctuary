...
describe("handleWorkflowMethod", () => {
  ...
  it("workflow.run 拒绝调用方选择 inline source", async () => {
    const result = await handleWorkflowMethod(
      {
        type: "req",
        id: "3-inline",
    ...
    expect(result?.ok).toBe(false);
    if (result && !result.ok) {
      expect(result.error.code).toBe("workflow_source_forbidden");
    }
    expect(mockRuntime.run).not.toHaveBeenCalled();
  });

  it("workflow.run 拒绝路径语义的 workflowName", async () => {
    const result = await handleWorkflowMethod(
      { type: "req", id: "3-path", method: "workflow.run", params: { workflowName: "../escape" } },
      { workflowRuntime: mockRuntime, stateDir: tempDir },
    );
