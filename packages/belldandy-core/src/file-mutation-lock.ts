...
export class FileMutationLockTimeoutError extends Error {
  readonly code = "file_mutation_lock_timeout";

  constructor() {
    super("Timed out waiting for the file mutation lock.");
    this.name = "FileMutationLockTimeoutError";
  }
}

export class FileMutationLockReleaseError extends Error {
  readonly code = "file_mutation_lock_release_failed";

  constructor() {
    super("Failed to release the file mutation lock.");
    this.name = "FileMutationLockReleaseError";
  }
}
...
function isCompleteOwner(
  owner: Partial<FileMutationLockOwner> | undefined,
): owner is FileMutationLockOwner {
...
async function recoverStaleLock(
  ...
  let owner: Partial<FileMutationLockOwner> | undefined;
  try {
    owner = JSON.parse(await fs.readFile(lockPath, "utf-8")) as Partial<FileMutationLockOwner>;
  } catch {
    owner = undefined;
  }
...
async function releaseLock(lockPath: string, token: string): Promise<void> {
  let lastOwner: Partial<FileMutationLockOwner> | undefined;
  for (let attempt = 1; attempt <= RELEASE_ATTEMPTS; attempt += 1) {
    try {
      const owner = JSON.parse(await fs.readFile(lockPath, "utf-8")) as Partial<FileMutationLockOwner>;
  ...
  throw new FileMutationLockReleaseError();
}

/**
 * 单文件跨进程 mutation owner。锁内容只保存释放所需的随机 token、pid 与时间戳。
 */
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new FileMutationLockTimeoutError();
      }
