...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (token) {
      ...
      case "--claude-command":
        if (next) {
          result.claudeCommand = next;
          index += 1;
        }
        break;
      case "--git-bash-path":
        if (next) {
          result.gitBashPath = path.resolve(next);
          index += 1;
        }
        break;
      case "--timeout-ms":
...
export async function executeClaudeExecOnce(launch, { prompt, model, cwd }) {
  ...
  const result = await runClaudeExec({
    claudeCommand: launch.claudeCommand,
    gitBashPath: launch.gitBashPath,
    cwd: resolvedCwd,
...
export async function executeClaudeTaskOnce(
  launch,
  {
    mode,
    objective,
  ...
  return executeClaudeExecOnce(launch, {
    prompt,
    model,
    cwd,
  });
...
function registerStructuredClaudeTool(server, launch, name, mode, description) {
  ...
  }, async (input) => executeClaudeTaskOnce(launch, {
    ...input,
    mode,
  }));
}

export function createClaudeBridgeServer(launch) {
  ...
  server.registerTool("exec_once", {
    description: "受控调用 Claude Code CLI 的一次性执行工具。适合 bridge mcp transport 试点，不提供交互式会话。",
    inputSchema: {
      prompt: z.string().min(1).describe("要提交给 Claude Code CLI 的任务指令"),
      model: z.string().optional().describe("可选模型名"),
      cwd: z.string().optional().describe("可选工作目录，必须位于启动时声明的 workspaceRoot 或 extraWorkspaceRoots 内"),
    },
    outputSchema: {
  ...
  }, async ({ prompt, model, cwd }) => executeClaudeExecOnce(launch, {
    prompt,
    model,
    cwd,
  }));
  ...
  server.registerTool("task_once", {
    description: "受控调用 Claude Code CLI 的结构化一次性执行工具。适合只读分析、代码审查和小范围改单文件。",
    inputSchema: {
  ...
  }, async (input) => executeClaudeTaskOnce(launch, input));

  registerStructuredClaudeTool(
    server,
    launch,
    "analyze_once",
    "analyze",
    "受控调用 Claude Code CLI 的只读分析工具。适合一次性结构化分析任务。",
  );
  registerStructuredClaudeTool(
    server,
    launch,
    "review_once",
    "review",
    "受控调用 Claude Code CLI 的代码审查工具。适合一次性 review 和结论输出。",
  );
  registerStructuredClaudeTool(
    server,
    launch,
    "patch_once",
    "patch",
    "受控调用 Claude Code CLI 的小范围改单文件工具。适合一次性有限修改任务。",
  );
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const server = createClaudeBridgeServer(launch);
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error(`[claude-bridge-server] running on stdio (workspaceRoot=${launch.workspaceRoot}, extraWorkspaceRoots=${launch.extraWorkspaceRoots.join(",")})`);
}
...
if (isMainModule) {
  main().catch((error) => {
    console.error("[claude-bridge-server] fatal:", error);
    process.exit(1);
  });
}
