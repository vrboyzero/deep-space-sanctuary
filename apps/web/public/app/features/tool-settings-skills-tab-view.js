...
function createToggle(ownerDocument, name, checked) {
  ...
  checkbox.setAttribute("data-category", "skills");
  checkbox.setAttribute("data-name", text(name));
  toggle.append(checkbox, createElement(ownerDocument, "span", "toggle-slider"));
  return toggle;
}

function compareSkillNames(left, right) {
  return text(left.name).localeCompare(text(right.name));
}

function appendTags(ownerDocument, info, tags) {
  if (!tags.length) return;
  const tagList = createElement(ownerDocument, "div", "skill-tags");
  tagList.append(...tags.map((tag) => createElement(ownerDocument, "span", "skill-tag", tag)));
  info.appendChild(tagList);
}

export function createToolSettingsSkillsTabView({ ownerDocument, t }) {
  return {
    render(target, { skills, enabledCount, toolControlView }) {
      if (!target) return null;

      const header = createElement(ownerDocument, "div", "tool-section-header");
      header.append(
        createElement(ownerDocument, "span", "", t("toolSettings.sectionSkills", {}, "Skills")),
        createElement(
          ownerDocument,
          "span",
          "tool-section-count",
          t(
            ...
            { enabled: enabledCount, total: skills.length },
      ...
      for (const skill of [...skills].sort(compareSkillNames)) {
        const row = createElement(
          ownerDocument,
          "div",
          `tool-item${skill.checked ? "" : " disabled"}${skill.visibility && !skill.visibility.available ? " unavailable" : ""}`,
        );
        const info = createElement(ownerDocument, "div", "skill-item-info");
        info.appendChild(createElement(ownerDocument, "span", "tool-item-name", skill.name));
        info.appendChild(createElement(ownerDocument, "span", "skill-meta", `${skill.source} · ${skill.priority}`));
        if (skill.description) info.appendChild(createElement(ownerDocument, "span", "skill-desc", skill.description));
        appendTags(ownerDocument, info, skill.tags);
        appendVisibilitySummary(ownerDocument, info, skill.visibility);
        row.append(info, createToggle(ownerDocument, skill.name, skill.checked));
        children.push(row);
      }
