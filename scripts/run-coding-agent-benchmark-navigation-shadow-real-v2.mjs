...
export const CODING_AGENT_BENCHMARK_NAVIGATION_SHADOW_REAL_V2_VERSION =
  "coding-agent-benchmark-navigation-shadow-real-v2/v1";

const TASK_ID = "real-js.bug-fix";
const ANALYSIS_VERSION = "coding-agent-benchmark-navigation-shadow-analysis/v1";
const PREVIOUS_SHADOW_VERSION = "coding-agent-benchmark-navigation-shadow-real/v1";
...
export function buildNavigationShadowRealV2Artifact(input) {
  ...
  const candidateEvidence = requireObject(input?.candidateEvidence, "candidate v2 evidence");
  const candidateEvidenceText = requireText(
    input?.candidateEvidenceText,
    "candidate v2 evidence text",
  );
  const analysisText = requireText(input?.analysisText, "P0.21 analysis text");
  const previousShadowArtifactText = requireText(
    input?.previousShadowArtifactText,
    "P0.20 shadow artifact text",
  );
  ...
  assertParsedTextMatches(candidateEvidence, candidateEvidenceText, "candidate v2 evidence");
  const analysis = parseJsonText(analysisText, "P0.21 analysis");
  const previousShadowArtifact = parseJsonText(
    previousShadowArtifactText,
    "P0.20 shadow artifact",
  );
  ...
  if (JSON.stringify(baselineSnapshotIdentity) !== JSON.stringify(candidateSnapshotIdentity)
    ...
    throw new Error("Navigation shadow real v2 repository snapshot identity drifted.");
  }

  if (run.taskId !== TASK_ID || run.platform !== platform || run.attempt !== 1) {
    throw new Error("Navigation shadow real v2 must contain one attempt-1 real-js.bug-fix result.");
  }
  if (run.environment?.model?.id !== modelId
    ...
    throw new Error("Navigation shadow real v2 model authorization drifted.");
  }
  if (codingCiManifest.mode !== "workspace-write"
    || codingCiManifest.profileCandidateId
      !== CODING_AGENT_BENCHMARK_NAVIGATION_CANDIDATE_V2_ID) {
    throw new Error("Coding CI did not execute the authorized navigation candidate v2.");
  }
  if (run.usage?.observation?.status !== "provider_reported"
    ...
    throw new Error("Navigation shadow real v2 requires provider-reported usage and cost.");
  }
  ...
  if (priorObservedCostCny + runCostCny > maxTotalCostCny + 1e-8) {
    throw new Error("Navigation shadow real v2 exceeded the authorized total CNY cost.");
  }
  ...
  if (startedTools.some((tool) => tool.name === "run_command")) {
    throw new Error("Navigation shadow candidate v2 invoked the denied run_command tool.");
  }
  ...
  if (renderedPromptSha256 !== candidateEvidence.prompt?.renderedPromptSha256) {
    throw new Error("Navigation shadow real v2 rendered prompt hash drifted.");
  }
  const promptContract = analyzePromptContract(startedTools);
  ...
  return {
    schemaVersion: CODING_AGENT_BENCHMARK_NAVIGATION_SHADOW_REAL_V2_VERSION,
    generatedAt,
    status: "completed",
    taskId: TASK_ID,
    platform,
    ...
    source: {
      ...
      analysisSha256: candidateEvidence.source.analysisSha256,
      previousShadowArtifactSha256: candidateEvidence.source.shadowArtifactSha256,
      navigationEvidenceSha256: candidateEvidence.source.navigationEvidenceSha256,
...
export async function runNavigationShadowRealV2(input, dependencies = {}) {
  const platform = requirePlatform(input?.platform);
  ...
  if (finalizeExistingExecution) {
    ...
    await assertPathAbsent(
      path.join(outputRoot, "navigation-shadow-real-v2.json"),
      "real shadow v2 artifact",
    );
  } else {
  ...
  const paths = {
    ...
    candidateEvidence: path.join(candidateEvidenceRoot, "navigation-candidate-v2.json"),
    analysis: path.join(analysisRoot, "navigation-shadow-analysis.json"),
    previousShadowArtifact: path.join(previousShadowRoot, "navigation-shadow-real.json"),
  ...
  if (candidateEvidence.platform !== platform) {
    throw new Error("Navigation shadow real v2 candidate evidence platform drifted.");
  }
  ...
  const artifact = buildNavigationShadowRealV2Artifact({
    platform,
...
export function parseNavigationShadowRealV2CliArguments(argv) {
...
function validateCandidateEvidence(input) {
  ...
  const expectedProfile = buildNavigationCandidateV2Profile(manifest);
  if (manifest.schemaVersion !== MANIFEST_VERSION
    || candidateEvidence.schemaVersion !== CODING_AGENT_BENCHMARK_NAVIGATION_CANDIDATE_V2_VERSION
    ...
    || candidateEvidence.candidate?.id !== CODING_AGENT_BENCHMARK_NAVIGATION_CANDIDATE_V2_ID
    || candidateEvidence.candidate?.manifestModified !== false
    || candidateEvidence.candidate?.strategy?.id !== STRATEGY_ID
    || candidateEvidence.candidate?.strategy?.enforcement !== "prompt_contract"
    || candidateEvidence.candidate?.strategy?.runtimeToolGuard !== false
    || candidateEvidence.execution?.mode !== "offline-replay"
    ...
    throw new Error("Navigation shadow real v2 requires eligible candidate v2 evidence.");
  }
  if (analysis.schemaVersion !== ANALYSIS_VERSION
    ...
    throw new Error("Navigation shadow real v2 historical evidence versions drifted.");
  }
  if (candidateEvidence.source?.analysisSha256 !== sha256(analysisText)
    || candidateEvidence.source?.shadowArtifactSha256 !== sha256(previousShadowArtifactText)
    || candidateEvidence.source?.navigationEvidenceSha256 !== sha256(navigationEvidenceText)) {
    throw new Error("Navigation shadow real v2 historical source hashes drifted.");
  }
  const manifestSha256 = sha256(manifestText);
  if (candidateEvidence.source?.manifestSha256 !== manifestSha256
    || report.suite?.manifestSha256 !== manifestSha256) {
    throw new Error("Navigation shadow real v2 frozen manifest binding drifted.");
  }
  if (candidateEvidence.prompt?.strategyId !== STRATEGY_ID
    || candidateEvidence.prompt?.enforcement !== "prompt_contract"
    || candidateEvidence.prompt?.runtimeToolGuard !== false
    || candidateEvidence.prompt?.basePromptSha256 !== candidateEvidence.source?.basePromptSha256
    || !isSha256(candidateEvidence.prompt?.contractSha256)
    || !isSha256(candidateEvidence.prompt?.renderedPromptSha256)) {
    throw new Error("Navigation shadow real v2 prompt evidence drifted.");
  }
}
...
function validateBaselineEvidence(input) {
  ...
  if (sha256(baselineManifestText) !== navigationEvidence.source?.baselineManifestSha256
    ...
    throw new Error("Navigation shadow real v2 historical baseline hashes do not match evidence.");
  }
  ...
  if (baselineManifest.runId !== navigationEvidence.source?.baselineRunId
    ...
    throw new Error("Navigation shadow real v2 historical baseline identity drifted.");
  }
}

function analyzePromptContract(tools) {
  ...
  return {
    compliant: fileGlobBeforeSourceRead
...
function cloneArtifactRefs(refs) {
  ...
  if (Object.keys(refs).length !== required.length) {
    throw new Error("Navigation shadow real v2 artifact references contain unknown entries.");
  }
  return result;
}
...
function parseNamedArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!flag?.startsWith("--") || value === undefined || value.startsWith("--")) {
      throw new Error(
        `Invalid navigation shadow real v2 argument near ${String(flag ?? "<end>")}.`,
      );
    }
    const key = flag.slice(2);
    if (!supported.has(key)) {
      throw new Error(`Unknown navigation shadow real v2 argument: ${flag}.`);
    }
...
function assertParsedTextMatches(value, text, label) {
  if (JSON.stringify(value) !== JSON.stringify(parseJsonText(text, label))) {
    throw new Error(`Navigation shadow real v2 ${label} object does not match its text.`);
  }
}
...
async function assertDirectory(target, label) {
  ...
  if (!stats?.isDirectory()) {
    throw new Error(`Navigation shadow real v2 ${label} must be a directory.`);
  }
}

async function assertFile(target, label) {
  const stats = await fs.stat(target).catch(() => null);
  if (!stats?.isFile()) throw new Error(`Navigation shadow real v2 ${label} must be a file.`);
}
...
async function assertPathAbsent(target, label) {
  ...
  if (stats) throw new Error(`Navigation shadow real v2 ${label} already exists.`);
}

function requireSingleRun(report) {
  if (!Array.isArray(report?.runs) || report.runs.length !== 1) {
    throw new Error("Navigation shadow real v2 execution report must contain exactly one run.");
  }
...
function requireTerminalType(events) {
  ...
  if (!["run.completed", "run.failed", "run.cancelled", "run.interrupted"].includes(type)) {
    throw new Error("Navigation shadow real v2 events require one terminal event.");
  }
  return type;
}
...
function requirePlatform(value) {
  ...
  if (!SUPPORTED_PLATFORMS.has(platform)) {
    throw new Error("Navigation shadow real v2 platform must be windows-native or wsl2-linux.");
  }
  return platform;
}
...
function requireString(value, label) {
  if (typeof value !== "string" || !value.trim()) {
    throw new Error(`Navigation shadow real v2 requires ${label}.`);
  }
  return value.trim();
}
...
function requireArray(value, label) {
  if (!Array.isArray(value)) throw new Error(`Navigation shadow real v2 requires ${label}.`);
  return value;
}
...
async function main() {
  const artifact = await runNavigationShadowRealV2(
    parseNavigationShadowRealV2CliArguments(process.argv.slice(2)),
  );
  console.log(
    `[coding-agent-navigation-shadow-real-v2] ${artifact.platform} ${artifact.outcome.status}; `
    + `costCny=${artifact.authorization.runCostCny}; compliant=${artifact.promptContract.compliant}`,
  );
}
...
if (invokedPath === path.resolve(scriptPath)) {
  main().catch((error) => {
    console.error(`[coding-agent-navigation-shadow-real-v2] failed: ${safeMessage(error)}`);
    process.exitCode = 1;
  });
}
