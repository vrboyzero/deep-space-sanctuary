...
代码发生显著变化后，不会自动索引；只有后续工作需要新鲜图谱时才显式重新索引。推荐使用完整、可审查的 PowerShell 命令：
...
Git 提交只触发“是否需要刷新”的评估，不直接启动索引。半自动的含义是：Agent 在提交、阶段结论或恢复工作时根据变更类型给出刷新建议；真正的索引仍是一次显式、串行、可验证的人工/Agent 操作。

| 提交后的变化 | 处理方式 |
| --- | --- |
| 新增、移动或删除源码模块；跨 package import/export、公开 Interface/数据契约、入口、注册或关键调用链变化 | 标记为“CBM 刷新候选”。在下一次需要图谱架构、调用链或影响面证据的任务开始前先执行 `index_status`；需要新鲜结果、状态非 `ready` 或新符号缺失时，再执行本节的手动索引。 |
| 文档、注释、测试/fixture-only、单文件局部修复，且没有改变可检索的模块边界、符号或依赖关系 | 不索引；继续以 `rg`、源码和测试作为当前事实来源。 |
| 已知图谱查询结果陈旧、节点/边计数异常，或新增符号查不到 | 不等待下一次提交，立即按本节手动索引并执行 4.4、5.2 的完整验证。 |

禁止用 Git `pre-commit`、`post-commit` hook、watcher 或后台定时任务直接调用 `index_repository`。这些机制会把小改动放大为频繁并发写入，违背本机的单次、串行和可审查约束。提交后的交付说明或交接记录可写“CBM 刷新建议”，但不得把未运行的索引表述为已完成。
