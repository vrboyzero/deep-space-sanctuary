...
| Wave 3 | 预算、取消与 lifecycle | 进行中（`OPT-P02` 已完成） | 截至 2026-07-17，`OPT-P02` 已将 owner token-usage 外发收敛为默认全局 4、每 endpoint 1 并发、最多 128 个 tracked key 的有界单飞/待发队列；同一摘要 key 仅保留一个 in-flight 与一个累计 pending，超时会 Abort，失败响应正文最多保留 2 KiB 且经脱敏，Map key、Doctor 与日志均不保留 URL/token。`system.doctor` 的 B02 运行资源快照已加入 `token_usage_upload` 水位；URL userinfo 也纳入安全输出脱敏。定向 Protocol/Core/Community 回归 4 个文件、27 项通过；本轮实现后的 workspace build 与全量 Vitest 已通过（428 个文件、2805 项通过，1 项跳过）。尝试将 owner 配置 endpoint 直接迁入严格 `OutboundRequestPolicy` 时，真实 Core 调用方的自托管 DNS endpoint 被拒绝；为保持现有私网/DNS 兼容，当前保留原生 fetch，并将带显式 trusted-private 配置迁移的严格出站策略登记为 `split_task`，不纳入当前持续推进。后续计划：先实施 `OPT-D05`，收敛 Gateway supervisor 的重复 signal listener、spawn `error`/`exit` 双结算、重启 timer 与 shutdown 清理；优先处理它是因为其 E1 证据已确认且直接支撑本 Wave 的 lifecycle 闭环。当前关键缺口是连续重启、spawn 失败、SIGINT/SIGTERM 与重复终态的故障注入证明；外部 Delivery Gate、Windows 资产和 GitHub Release 事项继续按既定 `defer` 边界不进入本轮队列。 |
