...
| Wave 0 | 基线与 Delivery Gate | 进行中（依赖 Gate 已远端通过，Docker patch 输入修复待复核） | 截至 2026-07-16，`private/main` 的发布链验证提交 `044042c` 的 Docker run `29494116824` 曾整体成功，Docker Hub `vrboyzero/star-sanctuary` 的 `main`、`latest`、`sha-044042c` tag 分别在 `2026-07-16T11:41:01Z`、`11:41:04Z`、`11:41:06Z` 更新；`peter-evans/dockerhub-description@v4` 的描述 PATCH 仍为 `Forbidden`，但已通过 `continue-on-error` 降为 warning。`312960c` 已将 Fastembed 的 `tar` 消费迁至 `7.5.20`：pnpm patch 将不兼容的 `import tar from "tar"` 改为 namespace import，锁文件不再含 `tar@6.2.1`。本地 frozen install、Fastembed 实际 ESM import、OSV Scanner v2.3.8（567 packages）和 dependency Gate 均为 `zero_findings`；3 个定向文件、20 项测试、workspace build，以及 release-light 的 4 个 `v0.5.4` 资产校验通过。对应 Quality Gates run `29499428969` 的 Dependency audit、WebChat（含 CSP/Trusted Types fixture）、Distribution 与 B00 已成功，Linux 全量测试在本次记录时仍运行。Docker run `29499428985` 在 `Build Docker image (test)` 失败，根因为 `deps`/`builder` stage 的 frozen install 前未复制 `patches/fastembed@2.1.0.patch`，日志为 `ENOENT /app/patches/fastembed@2.1.0.patch`；因此 Docker Hub 发布、tag Release 与 Windows job 均按依赖关系跳过，未创建 tag 或 Release，也未发布 Windows 资产。已以先失败后通过的 Docker 契约测试固定该边界，并在两个 stage 的 frozen install 前增加 `COPY patches ./patches`；该测试 10 项通过，`corepack pnpm build` 通过。本机 Docker daemon 未运行，故无法进行本地 image build，最终容器验证以 GitHub Runner 的下一次 run 为准。`ENABLE_WINDOWS_PACKAGING` 继续未配置，Windows portable、winget 与 single-exe 均 `defer` 到 Wave 6/R06。后续计划：提交并按文件范围说明后仅推送 Docker patch 输入修复到 `private/main`，复核新 commit 的完整 Quality Gates、Docker test image/health smoke、多平台 build 与 Docker Hub tag；这是当前关键闭环。随后完成 branch protection 与 release signing/attestation；在单独 tag 发版授权下再核验 GitHub Release 的 4 个 release-light 附件及公开资产 hash 回读。上述外部闭环未完成前，不能声明 Delivery Ready。 |
