...
| P1-A | 可观测与基础一致性 | 进行中（OPT-P01 已完成） | `OPT-P01` 已让 Distribution 的 `runtime-paths`、portable 与 single-exe 三个运行入口统一使用 `@belldandy/protocol` 的 state-dir contract，并删除重复实现；新增结构契约测试，Protocol 行为测试与 Distribution 相邻回归共 3 个文件、14 项通过，`corepack pnpm build` 及全量 Vitest（424 个文件、2778 项通过，1 项跳过）通过。下一切片先实施 `OPT-MCP02` 的原子、串行、权限受限的 `mcp.json` 写入，因为它直接消除并发配置丢失与半文件风险；随后完成 B01-B03 指标。当前仍缺 MCP02 的并发/故障注入 fixture 与 B01-B03 的阶段、资源、Web 指标闭环；外部 Delivery Gate 继续按 Wave 0 延后至准备更新 `origin/main` 时统一恢复。 |
