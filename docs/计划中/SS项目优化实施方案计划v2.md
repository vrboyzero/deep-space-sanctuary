...
| P1-A | 可观测与基础一致性 | 进行中（OPT-P01、OPT-MCP02、OPT-B01 已完成） | `OPT-P01` 已让 Distribution 的 `runtime-paths`、portable 与 single-exe 三个运行入口统一使用 `@belldandy/protocol` 的 state-dir contract，并删除重复实现；新增结构契约测试，Protocol 行为测试与 Distribution 相邻回归共 3 个文件、14 项通过。`OPT-MCP02` 已让同一 Gateway 进程内的 `mcp.json` add/update/remove 串行 read-modify-write 并递增 `revision`，以同目录私有临时文件、flush、原子 rename 及 Windows 受控备份/恢复替代直接覆盖；读取历史文件会修复私有权限，配置文件/Server/args/env/header 均有硬上限且日志不输出 secret。MCP 包 7 个文件、58 项测试，以及并发、故障注入、Windows fallback、权限、容量 6 项 config fixture 通过。`OPT-B01` 已在 `QueryRuntimeTraceStore` 按 method、previousStage、stage、outcome 维护固定桶、有界滚动样本和有界聚合键，输出 count/sum/max/p50/p95 与慢阶段排行；`enableStageDurationAggregation: false` 可关闭聚合但保留原始 trace/stop diagnostics，detail 不进入聚合。`system.doctor` 已返回 `stageDurations`，WebChat Query Runtime 卡片与文本摘要均展示最慢阶段 p95。固定时间戳、滚动截断、键淘汰、关闭兼容和敏感 detail 隔离均有回归；Query Runtime 9 项、Doctor 41 项、WebChat 12 项定向测试通过，`corepack pnpm build` 与全量 Vitest（424 个文件、2786 项通过，1 项跳过）通过。后续计划：先实施 `OPT-B02` 的事件循环、进程资源和真实队列快照，优先区分“阶段慢”与“资源/背压慢”的根因，并为 Wave 3 的预算、取消与 lifecycle 提供统一水位；当前关键闭环是 B02/B03 的资源与 Web 交互指标。跨进程共享同一 state dir 的文件锁不在 MCP02 的单进程配置仓储范围内；外部 Delivery Gate 继续按 Wave 0 延后至准备更新 `origin/main` 时统一恢复。 |
