...
| Wave 0 | 基线与 Delivery Gate | 进行中（`private/main` CI 与 Docker 发布已远端验证；外部 Delivery Gate 延后到 `origin/main` 准备期） | 截至 2026-07-16，`312960c` 通过 Fastembed pnpm patch 将不兼容的 `import tar from "tar"` 改为 namespace import，并将 `fastembed@2.1.0>tar` 精确覆盖到 `7.5.20`，锁文件不再含 `tar@6.2.1`。本地 frozen install、实际 ESM import、OSV Scanner v2.3.8（567 packages）与 dependency Gate 均为 `zero_findings`；3 个定向文件、20 项测试、workspace build，以及 release-light 的 4 个 `v0.5.4` 资产校验通过。首轮 Docker run `29499428985` 暴露 `ENOENT /app/patches/fastembed@2.1.0.patch`；`1e6ee5a` 在 `deps`/`builder` stage 均增加 `COPY patches ./patches`，并用先失败后通过的 Docker 契约测试固定顺序（10 项通过）。最终 Quality Gates run `29500079543` 与 Docker run `29500081919` 整体成功：Dependency audit、WebChat CSP/Trusted Types fixture、Distribution、B00、workspace build、Linux 全量测试、test image、health smoke、多平台 build 与 Docker Hub push 均通过；`main`、`latest`、`sha-1e6ee5a` 均回读到 digest `sha256:2a486edee941fc2c874754d1367d0e53f24fad3d82a52c1b08d4776d65e8964a`。branch push 未创建 tag 或 GitHub Release，tag Release 与 Windows job 按条件跳过；`ENABLE_WINDOWS_PACKAGING` 继续未配置，Windows portable、winget 与 single-exe 均 `defer` 到 Wave 6/R06。Repository Admin 身份和 `repo/workflow` scope 已核验，但 GitHub REST 对私有仓库的 branch protection 与 rulesets 均返回 `403`，私有/internal GitHub artifact attestation 亦要求 GitHub Enterprise Cloud。按 owner 决策，仓库保持私有且暂不升级付费计划；branch protection、rulesets、GitHub artifact attestation、semver tag、GitHub Release 的 4 个 release-light 附件及公开 hash 回读统一裁决为 `defer`，不以 workflow 绕过、弱化 Gate 或伪造签名。该外部闭环在本文其余计划完成、准备更新 `origin/main` 时再恢复处理；在此之前，已通过的远端 CI/Docker 证据作为后续 P1 启动依据，但不宣称强制 Delivery Ready。后续计划：从 P1-A 可观测与基础一致性开始，随后按 Wave 3 的预算、取消与 lifecycle、Wave 4 状态/事务/retention、Wave 5 基准驱动的热路径体验优化、Wave 6 发行矩阵与 rollout 依序推进；进入 `origin/main` 准备期后，再统一核验外部 Gate 与公开发布资产。 |
...
| P1-A | 可观测与基础一致性 | 已完成 | `OPT-P01` 已让 Distribution 的 `runtime-paths`、portable 与 single-exe 三个运行入口统一使用 `@belldandy/protocol` 的 state-dir contract，并删除重复实现；新增结构契约测试，Protocol 行为测试与 Distribution 相邻回归共 3 个文件、14 项通过。`OPT-MCP02` 已让同一 Gateway 进程内的 `mcp.json` add/update/remove 串行 read-modify-write 并递增 `revision`，以同目录私有临时文件、flush、原子 rename 及 Windows 受控备份/恢复替代直接覆盖；读取历史文件会修复私有权限，配置文件/Server/args/env/header 均有硬上限且日志不输出 secret。MCP 包 7 个文件、58 项测试，以及并发、故障注入、Windows fallback、权限、容量 6 项 config fixture 通过。`OPT-B01` 已在 `QueryRuntimeTraceStore` 按 method、previousStage、stage、outcome 维护固定桶、有界滚动样本和有界聚合键，输出 count/sum/max/p50/p95 与慢阶段排行；`enableStageDurationAggregation: false` 可关闭聚合但保留原始 trace/stop diagnostics，detail 不进入聚合。`system.doctor` 已返回 `stageDurations`，WebChat Query Runtime 卡片与文本摘要均展示最慢阶段 p95。`OPT-B02` 新建 Core 单例 `RuntimeResourceObservability`，默认每 15 秒采样、保留 24 条，可通过环境变量关闭；它以只读、无 GC 的方式采集 event-loop delay/利用率、RSS/heap，并从 SubAgent、Workflow、Cron、durable extraction、Webhook idempotency 与 WebSocket 获取仅含受控标签和数值的 active/queued 快照。队列输出同时限制为最多 16 个 provider、每 provider 16 条、总计 32 条，避免诊断输入反向放大内存或 Doctor payload。Gateway timer 已 `unref` 并在 close 时停止；`system.doctor` 新增 `runtimeResources` 与 `runtime_resources` 检查，WebChat 以双语运行资源卡片和文本摘要展示窗口、事件循环、内存与队列水位，关闭采样时明确告警而不伪报等待。采样截断、关闭、真实 Doctor/WebSocket 输出、运行时快照和前端渲染均有回归；`corepack pnpm build`、`corepack pnpm verify:webchat` 与全量 Vitest（425 个文件、2790 项通过，1 项跳过）通过。本轮全量日志仍偶发 `conversation.digest` 在临时 state dir 清理后写 `usage-accounting` 的非失败告警，判定为既有 teardown/atomic-write 生命周期债务，`defer` 至 Wave 3/4，不影响 B02 的资源观测边界或测试结论。`OPT-B03` 已新增有界 `webchat-performance-observability`：记录首屏 startup、流式 Markdown 渲染、Long Task 与 interaction observer 的受控短窗口，只保留阶段名、字符数、耗时和受控交互名；User Timing marks 使用后清理，浏览器不支持 observer 时静默降级，`pagehide` 释放 observer 与启动订阅。启动 bootstrap 也将既有和新增标记限制为 64 条、每条最多 16 个数值/布尔字段，禁止 retain referer、URL、消息正文或 DOM 内容；Doctor chat 与 Settings 仅注入本页局部性能摘要。10/100/1000 字符流式 fixture、历史标记净化、observer 生命周期及 Doctor 渲染回归通过；`corepack pnpm build`、`corepack pnpm verify:webchat`（128 文件）与全量 Vitest（427 文件、2797 项通过，1 项跳过）通过。全量过程在 23:59 暴露 Heartbeat 全天夹具把排他 `23:59` 当成全天结束时间，已改为 `24:00` 并以固定时钟回归，不改变生产 active-hours 语义。后续计划：P1-A 已闭合，下一步进入 Wave 3，先实施 `OPT-P02` 收敛 token-usage 出站的单飞/待发队列、响应体上限与不可逆 key；优先做它是因为 B02 资源观测和 Wave 2 的出站策略已可复用，而当前还缺的关键闭环是 timeout 后真实停止工作、队列上限和 shutdown/drain 的故障注入证明。跨进程共享同一 state dir 的文件锁不在 MCP02 的单进程配置仓储范围内；外部 Delivery Gate 继续按 Wave 0 延后至全部计划完成、准备更新 `origin/main` 时统一恢复。 |
| Wave 3 | 预算、取消与 lifecycle | 进行中（`OPT-P02`、`OPT-D05` 已完成） | 截至 2026-07-17，`OPT-P02` 已将 owner token-usage 外发收敛为默认全局 4、每 endpoint 1 并发、最多 128 个 tracked key 的有界单飞/待发队列；同一摘要 key 仅保留一个 in-flight 与一个累计 pending，超时会 Abort，失败响应正文最多保留 2 KiB 且经脱敏，Map key、Doctor 与日志均不保留 URL/token。`system.doctor` 的 B02 运行资源快照已加入 `token_usage_upload` 水位；URL userinfo 也纳入安全输出脱敏。尝试将 owner 配置 endpoint 直接迁入严格 `OutboundRequestPolicy` 时，真实 Core 调用方的自托管 DNS endpoint 被拒绝；为保持现有私网/DNS 兼容，当前保留原生 fetch，并将带显式 trusted-private 配置迁移的严格出站策略登记为 `split_task`，不纳入当前持续推进。`OPT-D05` 已新建共享 `gateway-supervisor-lifecycle`：它在 Distribution portable/single-exe 与 Core `bdd start/dev` 两个前台入口中保持唯一 active child、唯一 restart timer 和唯一 `SIGINT`/`SIGTERM` listener pair；每个 child 的 `error`/`exit` 只允许结算一次，shutdown 会取消等待重启、移除 listener 并清理 PID。连续重启、迟到 error、spawn error、signal 与待重启 shutdown 及 Core 装配均有回归。全量首次运行暴露 WebSocket pairing fixture 只尝试最后一个 code 的偶发竞态，已复用既有多 code/重试 helper，未改变生产行为。定向 3 个文件、44 项和 Distribution 18 个文件、92 项通过；workspace build 与全量 Vitest 通过（429 个文件、2810 项通过，1 项跳过）。`conversation.digest` 临时 state-dir 清理后的非失败 `ENOENT` 告警仍是既有 Wave 3/4 lifecycle/atomic-write 债务。PID instance nonce/进程创建时间的跨平台 ownership 加固需独立 process metadata fixture，登记为 `split_task`，不纳入当前持续推进。后续计划：先实施 `OPT-BR03`，将 Browser Relay/扩展的重连、socket、generation、listener 与 stop ownership 收敛到单一控制器；优先处理它是因为其 E1 重连 listener 累积与重复事件已确认，且能复用本轮 child lifecycle 的单终态/单 timer 经验。当前关键闭环是 20 次断连重连、旧 socket 迟到 close、Extension suspend/disable 与 Relay stop/drain 的故障注入证明；外部 Delivery Gate、Windows 资产和 GitHub Release 事项继续按既定 `defer` 边界不进入本轮队列。 |
...
1. 推进开发时，如遇到超过 3000 行的大型代码文件，优先采用不继续扩容该文件的实现方式；确有必要在原文件接入时，应将新增功能尽量拆分到相邻模块，由原文件只保留必要的装配、注册或转发。本计划当前不要求缩减既有文件体量，但应尽量减缓超过 3000 行文件的继续增长。
2. 执行本计划期间，持续保持“开发 → 测试 → 完成阶段任务时回写进度说明 → 再开发”的闭环，直至本文计划全部完成或用户明确叫停。
3. 计划中已明确标记为 `defer`、延期或暂不实施的内容，不纳入当前持续推进队列；仅在新的证据改变优先级或用户明确恢复该事项时重新进入计划。
