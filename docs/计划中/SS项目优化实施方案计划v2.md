...
| Wave 0 | 基线与 Delivery Gate | 进行中（第二轮本地修复完成，待远端复核） | 截至 2026-07-16，`private/main@164d3e7` 的第二轮远端证据已结束：WebChat 123-file contract（含 enforced CSP / Trusted Types fixture）、Distribution contract、B00 build benchmark，以及 Docker test image、health smoke、多平台 image build 均通过。Quality Gates 的 Linux 全量从首轮 6 项失败收敛为 1 项：canonical profile state 已正确注入，但 metadata 在同时满足摘要信号阈值时错误标记为 `signal_threshold`，而不是 `profile_state_present`；其余 Gateway/PTY 回归均已消失。Dependency audit 仍因真实 `tar@6.2.1` finding 为 `findings_present`，按 fail-closed 设计正确红灯。Docker Publish to Docker Hub 仍在登录时返回 `Username and password required`，Create GitHub Release 与 Windows Packaging Assets 被跳过。本地已修复 profile activation 原因优先级，并新增“canonical state 与 signal threshold 同时成立”回归；`pnpm build`、profile 4 项、Gateway 18 项定向测试、Windows 全量 Vitest（422 文件/2774 测试、1 项跳过）均通过。当前工作树无法由 WSL 直接复跑最新 Linux 测试，原因是 Windows `node_modules` 缺少 `@rollup/rollup-linux-x64-gnu`；Linux 全量结论以新 GitHub Actions 为准。Dependency audit finding、Docker Hub 凭据、branch protection、release signing/attestation 与公开发布资产回读仍未闭环。后续计划：提交仅含 `belldandy-core` profile 修复与本表更新的最小增量，推送 `private/main` 后复核新 Quality Gates/Docker；先做此回推是因为它是当前唯一代码层 Linux 失败，修复后才可将剩余红灯明确归为依赖 finding 和 Repository Admin 配置。 |
