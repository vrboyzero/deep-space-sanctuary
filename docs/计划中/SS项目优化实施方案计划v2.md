...
| Wave 0 | 基线与 Delivery Gate | 进行中（代码 Gate 已远端复核，外部 Delivery Gate 阻塞） | 截至 2026-07-16，修复提交 `e321cee` 已推送到 `private/main`，并由 `git ls-remote` 确认。其 Quality Gates run `29484201873` 中，Linux Build and full test suite、WebChat 123-file contract（含 enforced CSP / Trusted Types fixture）、Distribution contract 与 B00 build benchmark 全部通过；此前 `workspace-build-guard.test.ts` 的 source/dist mtime 同刻误判已关闭。本地定向回归连续 20 次通过，`corepack pnpm build` 通过，完整 Vitest 为 422 files/2774 tests 通过、各 1 项跳过。Quality Gates 整体仍仅因真实 `tar@6.2.1` finding 的 Dependency audit 以 `findings_present` fail-closed。Docker run `29484201849` 的 Build & Test 已通过 test image、health smoke 与多平台 image build；Publish to Docker Hub 仍在 `docker/login-action@v3` 登录步骤报 `Username and password required`，使 image push、GitHub Release 与 Windows Packaging Assets 跳过。本机 `docker ps` 无法连接 `npipe:////./pipe/dockerDesktopLinuxEngine`，但不影响上述远端结论。当前 `ENABLE_WINDOWS_PACKAGING` 未配置，GitHub Release 只包含 release-light ZIP/TAR/manifest/checksum；Windows portable、winget 预备产物与 single-exe 均明确 `defer` 到 Wave 6/R06，不作为 Wave 0 发布闭环或 GitHub 发布目标。Docker Hub 凭据、依赖 finding 处置、branch protection、release signing/attestation 与 release-light 公开资产回读仍未闭环，不能声明 Delivery Ready。后续计划：本进度回写作为文档提交推送到 `private/main`；代码修复的远端 Gate 已完成复核，不对文档提交递归新增实现结论。随后先由 Repository Admin 配置 Docker Hub 用户名和 token，仅恢复 Docker/release-light 发布链；保持 `ENABLE_WINDOWS_PACKAGING` 未启用。再按上游 Fastembed/Tar 处置计划复核 finding，并完成 branch protection、签名/attestation 与 release-light 公开资产回读，作为 Wave 0 最终关闭条件。 |
