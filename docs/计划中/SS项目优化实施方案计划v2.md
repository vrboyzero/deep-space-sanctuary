...
| P1-A | 可观测与基础一致性 | 进行中（OPT-P01、OPT-MCP02 已完成） | `OPT-P01` 已让 Distribution 的 `runtime-paths`、portable 与 single-exe 三个运行入口统一使用 `@belldandy/protocol` 的 state-dir contract，并删除重复实现；新增结构契约测试，Protocol 行为测试与 Distribution 相邻回归共 3 个文件、14 项通过。`OPT-MCP02` 已让同一 Gateway 进程内的 `mcp.json` add/update/remove 串行 read-modify-write 并递增 `revision`，以同目录私有临时文件、flush、原子 rename 及 Windows 受控备份/恢复替代直接覆盖；读取历史文件会修复私有权限，配置文件/Server/args/env/header 均有硬上限且日志不输出 secret。MCP 包 7 个文件、58 项测试，以及并发、故障注入、Windows fallback、权限、容量 6 项 config fixture 通过；`corepack pnpm build` 与全量 Vitest（424 个文件、2783 项通过，1 项跳过）通过。下一切片实施 `OPT-B01` 的 QueryRuntime 阶段耗时聚合，先统一指标语义和有界聚合面，供 B02/B03 复用；当前关键闭环是 B01-B03 的阶段、资源、Web 指标。跨进程共享同一 state dir 的文件锁不在 MCP02 的单进程配置仓储范围内；外部 Delivery Gate 继续按 Wave 0 延后至准备更新 `origin/main` 时统一恢复。 |
