...
| Wave 0 | 基线与 Delivery Gate | 进行中（代码 Gate 通过，外部 Delivery Gate 阻塞） | 截至 2026-07-16，`private/main@57ef3df` 已完成第二轮远端复核。Quality Gates：WebChat 123-file contract（含 enforced CSP / Trusted Types fixture）、Distribution contract、B00 build benchmark、Linux Build and full test suite 全部通过；首轮 6 项 Linux 回归已全部关闭。唯一 failure 是 Dependency audit 的真实 `tar@6.2.1` finding，`findings_present` 保持 fail-closed。Docker Build & Publish：test image、health smoke、多平台 image build 全部通过；Publish to Docker Hub 的登录返回 `Username and password required`，因此 image push、GitHub Release 与 Windows Packaging Assets 被跳过。代码层闭环已完成；Docker Hub 凭据、依赖 finding 处置、branch protection、release signing/attestation 与公开发布资产回读仍未闭环，不能声明 Delivery Ready。后续计划：先核对 `tar@6.2.1` 的 transitive owner 并以最小依赖升级/替换消除 audit finding；Repository Admin 同步配置 Docker Hub 用户名和 token，再重跑发布 job。之后核验 branch protection、签名/attestation 与公开资产回读，作为 Wave 0 的最终关闭条件。 |
