...
后继 `cb01ccd` Web Windows 唯一 formal 已执行、失败并冻结：Agent/CLI 为 `exit 0 / run.completed`，但冻结 evaluator 正确给出 `failed/product_workflow`、tests/regression=`true/0`、patchAccepted/taskCompleted=`false/false`。确定性重放确认首次 objective review 与 input retry 均拿到 mutation 后当前条件；真实缺口是 correction 只重复已有注释和未变化源码行仍可进入 executor，并在下游复核中被错误宣称完成。冗余 correction 执行前 Gate 已由 `abe40b1` 新 identity formal 真实证明生效。

`abe40b1` Windows 唯一 formal 同样已执行、失败并冻结：Agent 首次 mutation 错误放宽全部 `false`，objective correction 随后恢复 aria 行为且确定性测试通过，但同时把已正确的 `popover` 三元表达式展开为分支；冻结 evaluator 因未满足“最小恢复上游条件”拒绝 patch。后继 `dd6b85b` 虽已强化 objective correction 对相邻正确代码的最小保留合同，但唯一 Windows formal 在首次 mutation 就再次放宽全部 `false`，完整复读后的 objective review 误判完成且没有发起 correction，冻结 evaluator 正确拒绝。该样本进一步排除了 stale context、复读证据投影缺失和 correction 输入构造问题，当前缺口收敛为“子集正例通过后，objective review 未验证未请求行为保持不变”。
...
2. 永久冻结 `cb01ccd`、`abe40b1` 与 `dd6b85b` 已执行 dry-run/formal，不为失败 identity 启动 WSL2。
3. 子集行为保持合同已在公开 Tool Agent seam 测试先行闭合；下一步形成新 identity 并完成全部零模型 Gate，通过后才评估一次新的 Windows formal；不改 evaluator、证据投影、Tool 状态机或执行预算。
...
#### P0 Web 代表验证结论：`09b5498` Windows formal（2026-08-18）

##### 已完成内容

1. **唯一 Windows formal 执行并冻结**：
   - artifact=`artifacts/p0-web-local-objective-retry-canary-09b5498-preact-windows-formal-r1`，run=`real-web-ui-regression-windows-a1-1787032488744`，report SHA-256=`e3c6dbb6e1f325303622dbd4439d5d13be980f2690a44811ad7bd997f872b989`；
   - source/harness 均固定为 `09b5498c9212ba6d39173a2108017d330cb09486`，唯一终态=`run.failed`；event/trace=`17/19`、`result.json=null`，该 identity 禁止重跑或启动 WSL2；
   - route/model 固定为 `deepseek-v4-flash`，Provider usage=`5/5` complete、input/output=`10,577/850` tokens、实际费用=`$0.00353063`。

2. **pre-execution 恢复生效，但两次 correction 均未形成可执行上下文**：
   - 初始 patch 只为 `aria-*` / `data-*` 的布尔 `false` 增加序列化分支，冻结测试通过，但没有恢复 evaluator 要求的 `value != NULL && (value !== false || name[4] == '-')` 语义；
   - 第一次 objective correction 通过本地 required-path/valid-section 校验，进入 executor 后因目标上下文不匹配返回可信 `apply_patch_input_invalid`，既有一次 input correction 被正确触发；
   - 第二次 correction 仍引用不匹配的上下文并返回同一可信 input error，状态机按合同立即失败关闭；两个失败 patch 都未执行，没有追加复读、final review、turn、token 或 Provider retry。

3. **冻结 evaluator 与 patch 证据完成**：
   - changes patch SHA-256=`86219fa848487f1ae278035e0c7f19012d932bf0d1bc1656f591c35bc5e0fcd5`，只修改 `src/diff/props.js`，fixture untracked=`0`；
   - benchmark 与独立冻结 evaluator 均得到 tests=`true`、regression=`0`、patchAccepted/taskCompleted=`false/false`，独立 evaluator 同样输出 `3` 条 diagnostics；
   - failureCategory=`product_workflow`，不是 infrastructure、usage、readiness、schema 或敏感值失败。

4. **readiness、费用、敏感值与资源审计完成**：
   - Gateway readiness SHA-256=`2ba0ee7702296ac27c1780f9b6241a72dbacfabc961f74a49e74ddcde8035c73`，首 stdout/端口/认证=`2.035/10.807/10.815s`、stderr=`0 bytes`，stop 后 `15ms` 受控退出；
   - artifact/fixture/runtime 扫描常规文件=`8,376`，unreadable/真实 Provider key 精确命中=`0/0`；
   - formal 新生成 `.env` / `.env.local` 经 containment、普通文件、无 reparse point 与 SHA-256 校验后，已按持续授权送入 Windows 回收站；cleanup log=`tmp/p0-web-local-objective-retry-canary-09b5498-preact-windows-formal-r1-sensitive-cleanup.log`，listener/相关 Node/剩余 env=`0/0/0`；
   - observed conservative upper 更新为 `$2.43762086`，一般费用守卫=`33.43864688 RMB < 50 RMB`，单 run、Stage 0D、turn/token/retry 合同均未放宽。

5. **效果**：
   - `09b5498` 真实证明 executor 前的本地拒绝恢复已接通，首次 correction 可进入 executor，并在可信 input error 后消费唯一一次有界 input correction；
   - formal 同时证明“增加一次输入纠正”不足以完成该任务：模型生成的 correction 上下文连续两次与当前 workspace 不匹配，测试通过也不能替代隐藏语义验收；
   - 技术债裁决：objective input correction 的 stale/incorrect context=`fix_now`；先做确定性复现和最小修复，不重跑 `09b5498`，不增加预算或 retry，不进入 WSL2、完整矩阵、candidate v4 或 P2-C。

##### 验证结果

- TypeScript 编译无错误：同 identity 的 detached `corepack pnpm build` 与独立 `corepack pnpm verify:build` 已在 formal 前通过；
- `171` 个 Agent 定向测试与 `28` 个 launcher/fixture 测试全部通过（本 formal 环节新增测试=`0`）；冻结任务测试和独立 evaluator 均确认 tests=`true`、regression=`0`；
- terminal、usage、patch、evaluator、readiness、费用、敏感值、env 回收站清理与资源收敛均已审计；formal 结论为产品工作流失败，不能表述为 Web 代表任务成功。

##### 后续计划

- **下一步准备做什么**：暂停后从确定性测试开始，复现 objective input correction 连续生成 stale/incorrect context 的路径，区分提示材料陈旧、复读证据投影和 correction 输入构造问题，再做最小修复；
...
#### P0 Web 代表准备实现结论：`cb01ccd` Windows 零模型 Gate（2026-08-19）

##### 已完成内容

1. **`tmp/p0-web-current-source-canary-cb01ccd-clean` detached clean harness 建立**：
   - source/harness 固定为 `cb01ccd0f89ba399eaa974f0cf67eff2f8910157`，workspace dirty=`false/false`，lockfile/content SHA-256=`844c0021...` / `c9154a30...`；
   - 首次 frozen offline install 在本地链接阶段触达 `300s` 工具上限，核查相关进程=`0`、Git clean 且依赖目录已建立后，同一命令幂等续跑退出 `0`，downloaded=`0`；
   - 完整 `corepack pnpm build`、独立 `corepack pnpm verify:build` 均通过，正确 Vitest 入口下 launcher/fixture 定向测试=`28/28`。

2. **Windows 零凭证 dry-run 完成**：
   - artifact=`artifacts/p0-web-current-source-canary-cb01ccd-preact-windows-dry-run-r1`，run=`real-web-ui-regression-windows-a1-1787080932685`，report SHA-256=`fa383b1829ca9c4a92dd61d12b7a00683366ac2951dbafbfb175f9b54e9f8456`；
   - benchmark/snapshot 双 preflight=`passed/passed`，source snapshot commit=`6bb8272...`，fixture HEAD=`0ccf7aa...` 且 dirty/untracked/diff=`0/0/0`；
   - credentials/model calls/usage=`false/0/not_reached`，event/trace/patch/changed paths=`0/0/0/0`，固定模型仍为 `deepseek-v4-flash`。

3. **readiness、敏感值与资源收敛完成**：
   - Gateway 首 stdout/端口/认证=`8.590/17.675/17.683s`，stderr=`0 bytes`，stop 请求后 `13ms` 受控退出；
   - artifact/fixture/runtime/repository input 扫描常规文件=`8,062`，unreadable/真实 Provider key 精确命中=`0/0`；
   - 新生成 `.env` / `.env.local` 经 containment、普通文件、无 reparse point 与 SHA-256 校验后送入 Windows 回收站，cleanup log=`tmp/p0-web-current-source-canary-cb01ccd-preact-windows-dry-run-r1-sensitive-cleanup.log`；listener/相关 Node/剩余 env=`0/0/0`。

4. **formal prepare-only 输入审计完成**：
   - formal repository input SHA-256=`2ef316c1cb316832068a1080e80c172d6ca30b24cda0652fec500734acd21d68`，绑定 dry-run receipt SHA-256=`0e74aee3cc8cbff687f95b61aa9c5d0cbfcac79cbd50e049ec07831b0c7ebe6d`；
   - 固定 `deepseek-v4-flash`、高峰价 cache/input/output=`0.0125/0.375/1.125 USD/1M`、Provider retry=`0` 与 `3.23762086 -> 3.33762086 USD` 累计窗口；
   - Provider key 已配置且未进入参数，Gateway/benchmark spawned=`false/false`，三个预定 formal 输出根、端口、相关 Node 均为 `0`。

5. **效果**：
   - `cb01ccd` 已通过 detached offline build、fixture/evaluator 合同、双 preflight、真实 Windows Gateway readiness/auth、固定 route、零凭证失败关闭、敏感值与资源 Gate；
   - 当前只开放一次新的 Windows formal，不重跑冻结 identity，不先启动 WSL2、完整矩阵、candidate v4 或 P2-C。

##### 验证结果

- TypeScript workspace 完整 build 与独立 `verify:build` 无错误；launcher/fixture `28` 个测试全部通过（本 Gate 新增测试=`0`）；
- dry-run 双 preflight、readiness/auth、fixture/harness clean、零 usage、空 event/trace/patch、敏感值扫描、env 回收站清理、资源收敛和 formal prepare-only Gate 全绿；
- 模型调用=`0`、新增费用=`$0`，费用守卫仍为 `33.43864688 RMB < 50 RMB`，最坏累计守卫仍为 `47.53768 RMB < 50 RMB`。

##### 后续计划

- **下一步准备做什么**：沿用已通过的 formal prepare-only 输入，固定 `deepseek-v4-flash`、高峰价、Provider retry=`0`、`12 turns / 24,000 tokens` 与累计费用窗口，执行且只执行一次 `cb01ccd` Windows formal；
- **为什么先做它**：全部无费用 Gate 已闭合，真实 Web mutation、current-source objective correction、再次复读和 final review 是验证本次修复的最小剩余证据；
- **当前还缺的关键闭环**：formal 的 mutation/tests/patch、合法唯一 terminal、完整 Provider usage/cost、真实敏感值与零残留审计；无论结果如何均永久冻结该 formal，Windows 未全绿不进入 WSL2，也不进入完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：冗余 objective correction 执行前 Gate（2026-08-19）

##### 已完成内容

1. **`artifacts/p0-web-current-source-canary-cb01ccd-preact-windows-formal-r1` 冻结与分层归因**：
   - run=`real-web-ui-regression-windows-a1-1787081387015`，report SHA-256=`2f6c7b84d0bcd4c5c09e146ae1c3099dd90b9760cf89c554f31394335c101348`；Agent/CLI 为 `exit 0 / run.completed`，benchmark 为 `failed/product_workflow`，两类终态保持分离；
   - tests/regression=`true/0`，patchAccepted/taskCompleted=`false/false`；Provider usage=`7/7`、input/output=`14670/707`、cost=`$0.00369824`，模型固定为 `deepseek-v4-flash`；
   - 初始 mutation 将条件放宽为 `value != NULL`，objective correction 又重复已有 aria/data 注释并删除后原样加回 `if` 行；最终 patch/events/trace SHA-256=`f96558d8...` / `258873b5...` / `3a7bfb2b...`，冻结 evaluator 正确拒绝。

2. **提示材料、复读投影与 correction 构造分层复核**：
   - `prompt.md` 已明确“最小恢复 false aria-* 序列化、保留公开行为、只改目标源码”，提示材料无需扩大；
   - mutation 后完整 `file_read` 已成功，按 formal 实际源码和 `1637/1380` token 预算精确重放 objective review/input retry，两者都包含当前 `} else if (value != NULL) {`，因此不存在 stale read 或首次 correction 投影漏掉关键条件；
   - prompt snapshot SHA-256=`be6f0387...` 证明冗余注释执行后的最终复核片段不再包含该条件；这是无效 correction 已落盘造成的下游投影退化，不是首次 correction 输入根因。根因收敛为模型构造了重复当前源码的 correction，而产品缺少执行前拒绝。

3. **`react-workspace-mutation.ts` 与 `tool-agent.ts` 修改**：
   - objective review/retry 指令明确 correction 必须改变任务相关行为，注释不能替代源码修正，未变化源码行必须作为 context；
   - 新增基于每个 required path 最新完整 `file_read` 的冗余 hunk 判定，仅识别“删除行全部原样加回，且完整新增块已连续存在于当前源码”的窄形状；真实替换、纯增删和当前源码尚不存在的插入不受影响；
   - 首次命中复用既有唯一 objective input correction，retry 再次命中则在 executor 前失败关闭；不增加模型调用、turn/token、Provider retry 或费用上限。

4. **`react-workspace-mutation.test.ts` 与 `tool-agent-workspace-mutation.test.ts` 扩展**：
   - 覆盖 formal 同形冗余 correction、真实条件替换、最新绝对路径复读覆盖旧证据及 Preact 大文件投影；
   - Agent 回归证明 executor 只接收初始 mutation 与真实语义 correction，第一次冗余输入进入唯一 retry，第二次冗余输入立即失败关闭。

5. **效果**：
   - 与 `cb01ccd` 同形的注释重复/未变化行 correction 不再污染 workspace，也不会进入证据退化后的错误完成路径；
   - current-source 选择与投影修复保持不变，失败边界收敛到 objective correction 的执行前语义门禁；
   - 技术债裁决：重复当前源码的 objective correction=`fix_now`；冗余 correction 落盘后的 final-review 投影退化=`record_only`，因为新 Gate 已在其上游阻断该状态，不为单一失败扩大投影算法。

##### 验证结果

- TypeScript 编译无错误：`corepack pnpm build:incremental` 与 `corepack pnpm verify:build` 均通过；
- 同口径 `7` 个 Agent/structured-output 测试文件共 `236/236` 通过（含本轮 `4` 个新增回归）；两个 owner 测试文件 `120/120` 通过，额外更宽集合 `237/237` 通过；
- `corepack pnpm verify:coding-benchmark`、`corepack pnpm verify:coding-ci` 与 `git diff --check` 均通过；
- artifact/fixture/runtime/formal input 共扫描 `8374` 个常规文件，unreadable/真实 Provider key 精确命中=`0/0`；formal `.env/.env.local` 已按授权送入 Windows 回收站，cleanup log 已记录，listener/相关 Node/剩余 env=`0/0/0`。

##### 后续计划

- **下一步准备做什么**：提交当前最小修复形成新 source identity，再建立 detached clean harness，完成 offline frozen install、完整 build、独立 verifier、launcher/fixture tests、Windows 零凭证 dry-run、双 preflight、readiness/auth、敏感值与资源收敛及 formal prepare-only；
- **为什么先做它**：`cb01ccd` 已永久冻结，确定性 Gate 只能证明本地门禁行为；只有新 identity 的干净零模型链通过后，才可安全验证真实 `deepseek-v4-flash` 是否生成语义 correction 或被正确失败关闭；
- **当前还缺的关键闭环**：新 identity 的全部零模型 Gate，以及其通过后唯一 Windows formal 的 patch/evaluator、合法终态、usage/cost、敏感值与零残留；在此之前不启动 WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：`abe40b1` Windows formal 与非最小 correction 归因（2026-08-19）

##### 已完成内容

1. **`abe40b1` detached harness 与零模型 Gate 完成**：
   - source/harness 固定为 `abe40b1fdea0a4bbb1772f37087198c541ee2229`，frozen offline install downloaded=`0`，完整 build、独立 `verify:build` 与 launcher/fixture `28/28` 均通过；
   - dry-run artifact=`artifacts/p0-web-redundant-correction-canary-abe40b1-preact-windows-dry-run-r1`，run=`real-web-ui-regression-windows-a1-1787084347675`，report SHA-256=`9b5135571a67aa65d231517673329b35b2355c298d38b6b7bd2ace7a71aa47a8`；
   - 双 preflight=`passed/passed`，credentials/model calls/usage=`false/0/not_reached`，event/trace/patch=`0/0/0 bytes`；Gateway 端口/认证=`9.908/9.916s`，停止耗时 `14ms`。

2. **formal prepare-only 与唯一付费 run 冻结**：
   - formal input SHA-256=`5ed433f0af7f7fd9115688611ae727b597d876cca50cf4ebc38b9555d76d7a65`，绑定 dry-run receipt SHA-256=`0e74aee3cc8cbff687f95b61aa9c5d0cbfcac79cbd50e049ec07831b0c7ebe6d`；prepare-only 的 Gateway/benchmark spawned=`false/false`；
   - formal artifact=`artifacts/p0-web-redundant-correction-canary-abe40b1-preact-windows-formal-r1`，run=`real-web-ui-regression-windows-a1-1787084744678`，report SHA-256=`c0c3da3ecb4b2f09123353d49fb41c1601472f9de0464f093f3083c4c0fec621`；
   - CLI/terminal=`exit 0/run.completed`，benchmark=`failed/product_workflow`，tests/regression=`true/0`，patchAccepted/taskCompleted=`false/false`；usage=`7/7`、input/output=`11775/658`、cost=`$0.00362468`；
   - events `1..56` 连续且唯一 terminal，workspace snapshot=`available/exact/non-truncated`、changed paths=`1`；patch/events/trace SHA-256=`7dafdbe...` / `146dd4b...` / `4c03a62...`。

3. **失败形状分层归因完成**：
   - 初始 mutation 把 `value != NULL && value !== false` 放宽为 `value != NULL`，会错误序列化全部普通 `false` 属性；objective review 使用 mutation 后完整当前源码并正确要求 correction；
   - correction 不是冗余 patch：它恢复 `value !== false`，为 `aria-*` 添加独立分支，确定性测试通过；因此 `cb01ccd` 的 stale/incorrect context 与冗余 correction 缺口均未复发；
   - correction 同时把已正确的 `popover` 三元表达式展开为等价分支，最终为 `7 insertions / 1 deletion`；冻结 evaluator 要求恢复原上游最小条件 `value != NULL && (value !== false || name[4] == '-')`，因此正确拒绝；
   - prompt 已明确 smallest change，最终复读片段也包含当前条件和展开后的 `popover` 分支；根因收敛为 objective correction 指令没有足够明确要求“只修当前仍错误的表达式，并把已正确相邻语句原样保留为 patch context”。

4. **敏感值、费用与资源收敛完成**：
   - dry-run/formal 分别扫描 `8059/8373` 个常规文件，unreadable/真实 Provider key/repository input 命中均为 `0/0/0`；
   - 两轮新生成 `.env/.env.local` 均经 containment、普通文件、无 reparse point 与 SHA-256 校验后送入 Windows 回收站，cleanup log 已记录；
   - formal Gateway 端口/认证=`10.216/10.228s`，停止耗时 `17ms`；最终 listener/相关 Node/剩余 env=`0/0/0`。

5. **效果**：
   - 冗余 correction 执行前 Gate 已获真实外部证据，`cb01ccd` 同形 correction 未再次进入 executor；
   - 新失败不再是“没有语义修正”，而是“语义修正扩大到已正确相邻代码”，后续可继续在 objective correction 合同内做最小修复；
   - 技术债裁决：非最小 objective correction=`fix_now`；不放宽冻结 evaluator，不增加 turn/token、Provider retry 或费用上限。

##### 验证结果

- TypeScript workspace 完整 build 与独立 `verify:build` 无错误；launcher/fixture `28` 个测试全部通过（本 Gate 新增测试=`0`）；
- formal 的冻结 Preact 测试通过、regression=`0`，但 evaluator 正确拒绝非最小 patch；唯一 terminal、完整 usage、snapshot、敏感值和资源 Gate 均已审计；
- 本 formal 新增费用=`$0.00362468`，费用守卫更新为 `33.49723024 RMB < 50 RMB`，最坏累计守卫=`47.59626336 RMB < 50 RMB`。

##### 后续计划

- **下一步准备做什么**：测试先行扩展 objective review 与 input retry 指令，明确 correction 只修改当前仍错误的最小表达式，禁止重构、展开、规范化或等价重写已正确相邻代码；随后跑 owner/同口径回归与 build/合同 Gate；
- **为什么先做它**：formal 已证明提示任务、当前源码复读、冗余 Gate、测试和终态均正常，剩余最小缺口位于 correction 输入约束；先强化该合同比新增通用语义启发式更窄、更可解释；
- **当前还缺的关键闭环**：失败测试、最小指令修复、完整回归、新 identity 零模型 Gate，以及其后唯一 Windows formal 的 evaluator 通过；未闭合前不启动 WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：objective correction 最小变更合同（2026-08-19）

##### 已完成内容

1. **`react-workspace-mutation.ts` 修改**：
   - objective review 与唯一 objective input retry 均要求 correction 相对当前源码生成最小 patch；
   - 已正确的相邻表达式和分支必须原样保留为 patch context，禁止重构、展开、规范化、现代化或等价重写已满足任务的代码；
   - 不修改证据投影算法、冻结 evaluator、Tool 选择、状态机或执行额度，不增加 turn/token、Provider retry 和费用上限。

2. **`react-workspace-mutation.test.ts` 与 `tool-agent-workspace-mutation.test.ts` 扩展**：
   - 在既有 Preact 大文件回归中验证 objective review 同时收到最小变更合同和 mutation 后当前 `value != NULL` 分支；
   - 在既有公开 Agent correction/input-retry 路径中验证两阶段实际模型请求均携带原样保留与禁止等价重写合同；
   - 两处合同断言在实现前精确得到 `2 failed`，最小指令修复后得到 `2 passed`；未新增测试 case，保持现有测试 seam。

3. **效果**：
   - 已把 `abe40b1` 暴露的失败收敛到 correction 输入合同，明确要求只修当前仍错误的最小表达式；
   - 提示材料、mutation 后完整复读和 task-relevant evidence 投影保持不变，避免为单一外部样本扩大通用启发式；
   - 技术债裁决：非最小 objective correction 的本地修复=`fix_now` 已完成，真实模型外部有效性继续由新 identity 唯一 formal 验证。

##### 验证结果

- TypeScript 编译无错误：`corepack pnpm build:incremental` 与 `corepack pnpm verify:build` 均通过；
- 两个 owner 测试文件 `120/120` 通过；同口径 Agent/structured-output 集合 `10` 个文件 `252/252` 通过（新增测试 case=`0`，扩展既有合同测试=`2`）；
- `corepack pnpm verify:coding-benchmark`、`corepack pnpm verify:coding-ci` 与 `git diff --check` 均通过；
- 轻量对抗性 review 确认 `1,637` token 同口径仍保留当前源码证据，且执行额度、Tool 路由、retry 和 evaluator 均未改变。

##### 后续计划

- **下一步准备做什么**：创建仅包含本轮 Agent 源码、两份测试和计划文档的本地提交，形成新 source identity；随后建立 detached clean harness，完成 frozen offline install、完整 build、独立 verifier、launcher/fixture `28/28`、Windows 零凭证 dry-run、敏感值与资源收敛及 formal prepare-only；
- **为什么先做它**：本地合同和回归已闭合，但只有固定新 identity 的干净 harness 能排除未提交工作区、旧产物或环境漂移，并在不调用模型的前提下验证 formal 输入；
- **当前还缺的关键闭环**：新 identity 的全部零模型 Gate，以及全部通过后唯一 Windows formal 的最小 patch、冻结 evaluator、合法终态、usage/cost、敏感值与零残留；未闭合前不启动 WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：`dd6b85b` Windows 零模型 Gate（2026-08-19）

##### 已完成内容

1. **`dd6b85b` detached clean harness 建立与构建**：
   - source/harness 固定为 `dd6b85b1ba46be63dfc6c75889ab2b1ecf495bee`，Git tracked workspace clean；canonical lockfile/content SHA-256=`844c0021...` / `ee8a3511...`；
   - `corepack pnpm install --offline --frozen-lockfile` 解析 `493` 个包，downloaded=`0`，完整 build 与独立 `verify:build` 均通过；
   - Windows launcher `17` 项、v3 fixture/evaluator `11` 项的要求子集=`28/28`，额外 v3 contract `8/8`，合计 `36/36` 通过。

2. **Windows 零凭证 dry-run 完成**：
   - artifact=`artifacts/p0-web-minimal-correction-canary-dd6b85b-preact-windows-dry-run-r1`，run=`real-web-ui-regression-windows-a1-1787086509468`，report SHA-256=`cdaaf81cc6f35c872a2aecef2cc2d0b35731dcb2e51fc1b92b133bf77e35c3fd`；
   - 双 preflight=`passed/passed`，credentials/model calls/usage=`false/0/not_reached`，event/trace/patch=`0/0/0 bytes`；
   - Gateway 端口/认证=`10.729/10.736s`，停止耗时 `16ms`；harness/source identity 均为 clean、exact `dd6b85b`。

3. **敏感值、资源与 formal prepare-only Gate 完成**：
   - 对 clean harness、artifact、fixture 和 runtime 扫描 `42,855` 个常规文件，跳过 `1,265` 个链接，unreadable/真实 Provider key/repository input 命中=`0/0/0`；
   - dry-run 新生成 `.env/.env.local` 经 containment、常规文件、无 reparse point 与 SHA-256 校验后送入 Windows 回收站，cleanup log 已记录；listener/相关 Node/剩余 env=`0/0/0`；
   - formal input SHA-256=`929e6787d05aebb73f505b8f441addc893af0707f588425dd2fac72329cd6ffb`，绑定本次 dry-run receipt SHA-256=`0e74aee3cc8cbff687f95b61aa9c5d0cbfcac79cbd50e049ec07831b0c7ebe6d`；prepare-only 的 Gateway/benchmark spawned=`false/false`；
   - formal 固定 `deepseek-v4-flash`、Provider retry=`0`、`12 turns / 24,000 tokens`、高峰价与累计费用窗口 `$3.24494378 -> $3.34494378`，Provider key 不进入命令参数。

4. **效果**：
   - 当前 identity 已通过调用模型前的全部干净构建、fixture、凭据隔离、双 preflight、readiness/auth、费用和敏感值 Gate；
   - dry-run 未产生模型事件或费用，formal 输入与 frozen repository snapshot 已精确绑定；
   - 只有一次 `dd6b85b` Windows formal 被开放；任一真实结果都将立即冻结，不重跑、不先启动 WSL2。

##### 验证结果

- TypeScript workspace 完整 build 与独立 `verify:build` 无错误；launcher/fixture/contract `36` 个测试全部通过（本 Gate 新增测试=`0`）；
- dry-run 双 preflight、零 usage、空 event/trace/patch、敏感扫描、env 回收站清理、资源收敛和 formal prepare-only 全绿；
- 模型调用=`0`、新增费用=`$0`，费用守卫仍为 `33.49723024 RMB < 50 RMB`，最坏累计守卫仍为 `47.59626336 RMB < 50 RMB`。

##### 后续计划

- **下一步准备做什么**：沿用已通过的 formal input，执行且只执行一次 `dd6b85b` Windows formal；模型固定 `deepseek-v4-flash`，Provider retry=`0`、`12 turns / 24,000 tokens` 和单 run `$0.10` 不变；
- **为什么先做它**：全部无费用 Gate 已闭合，真实 objective correction 是否只恢复上游最小条件并保留 `popover` 原表达式，是当前唯一未验证的外部行为；
- **当前还缺的关键闭环**：formal 的 patch/evaluator、合法唯一 terminal、完整 Provider usage/cost、真实敏感值与零残留审计；无论结果如何均永久冻结，未全绿不进入 WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：`dd6b85b` Windows formal 失败与归因（2026-08-19）

##### 已完成内容

1. **唯一 Windows formal 执行并冻结**：
   - artifact=`artifacts/p0-web-minimal-correction-canary-dd6b85b-preact-windows-formal-r1`，run=`real-web-ui-regression-windows-a1-1787086989045`；source/harness 均为 clean、exact `dd6b85b1ba46be63dfc6c75889ab2b1ecf495bee`；
   - CLI=`exit 0 / run.completed`，benchmark=`failed / product_workflow`，tests/regression=`true/0`，patchAccepted/taskCompleted=`false/false`；
   - 模型固定 `deepseek-v4-flash`，Provider retry=`0`，model calls=`5/5`，input/output=`11,946/520`，provider cost=`$0.00297676`，duration=`9,408ms`。

2. **提示、复读投影与 correction 路径完成分层归因**：
   - 初始工具序列只有 `list_files -> file_read(source) -> file_read(aria test) -> apply_patch -> file_read(full post-write source)`；唯一 patch 将 `value != NULL && value !== false` 改为 `value != NULL`，使全部普通 `false` 属性也进入序列化；
   - mutation 后 `src/diff/props.js` 已按 `5,676` bytes 完整复读，objective review 的 system prompt 同时包含 smallest patch、相邻正确表达式原样保留和禁止等价重写合同，因此不是 stale/incorrect read，也不是当前源码证据投影缺失；
   - 本 run 没有 objective correction 或 input retry Tool call，新增 correction 输入合同没有被使用；真实缺口在 correction 之前：aria 正例通过后，objective review 没有检查普通 `false` 与 `popover` 等未请求行为是否保持不变，并错误生成“smallest possible”完成摘要。

3. **证据与敏感状态收敛**：
   - result/patch/events/trace SHA-256=`e95b2110...` / `2313d603...` / `e75ee40a...` / `f0adfff4...`；events `1..73` 连续，唯一 `run.completed`，usage=`complete`；
   - 双 preflight=`passed/passed`，Gateway readiness/auth=`10,335/10,343ms`，停止耗时约 `14ms`；
   - clean harness、artifact、fixture 与 runtime 共扫描 `43,169` 个常规文件，跳过 `1,265` 个 reparse/link，unreadable/真实 Provider key/完整 repository-input blob 命中=`0/0/0`；
   - formal runtime 的 `.env/.env.local` 经 containment、普通文件、无 reparse point 与 SHA-256 校验后按持续授权送入 Windows 回收站；cleanup log 已记录，listener/相关 Node/剩余 env=`0/0/0`。

4. **效果**：
   - `dd6b85b` formal 已永久冻结，不重跑、不启动该失败 identity 的 WSL2；
   - correction 最小变更合同没有被本次首次错误 mutation 验证，不能据此宣称外部有效；
   - 下一修复范围收敛到首次 mutation / objective review 的子集行为保持合同，不扩大为通用语义推断或 evaluator 放宽。

##### 验证结果

- TypeScript workspace 完整 build、独立 `verify:build` 及 benchmark/CI 合同验证沿用同一 clean `dd6b85b` identity 的零模型 Gate，均无错误；
- formal 的 `1` 条 frozen test command 通过，但 machine evaluator 正确拒绝扩大普通 `false` 行为的 patch，因此本轮功能验证结论为失败；
- event/terminal/usage、patch、敏感值和资源审计均闭合；新增费用=`$0.00297676`，一般费用守卫=`33.52104432 RMB < 50 RMB`，最坏累计守卫=`47.62007744 RMB < 50 RMB`。

##### 后续计划

- **下一步准备做什么**：在公开 Tool Agent workspace-mutation 行为边界新增一个失败回归，模拟单个 aria 正例通过但普通 `false` 语义被扩大，并要求完整当前源码复读后的 objective review 不能误判完成；随后只收紧首次 mutation / objective review 的子集行为保持指令；
- **为什么先做它**：formal 已分别证明任务提示可达、mutation 后源码完整复读、correction 输入未触发；先修实际发生的 review 误判，比修改已闭合的证据投影或继续强化未执行的 correction retry 更窄；
- **当前还缺的关键闭环**：失败测试必须先红、最小指令修复后转绿，并通过 owner/同口径回归、build 和 benchmark/CI 合同 Gate；形成新 identity 后才重新评估零模型 Gate，未通过前不开放付费 formal、WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：子集行为保持复核（2026-08-19）

##### 已完成内容

1. **`react-workspace-mutation.ts` 修改**：
   - 新增一条共享的 subset behavior preservation 合同：命名子集、特殊情况或例外的单个通过样本不能证明整体完成；
   - objective review 必须确认当前条件仍区分目标子集，并保持子集外输入行为，禁止把请求行为泛化到全部输入或折叠原排除条件；
   - 同一合同复用于唯一 objective input retry 与 correction 后 final review，避免恢复路径丢失；未修改证据投影、Tool 路由、状态机、turn/token、Provider retry 或 evaluator。

2. **`tool-agent-workspace-mutation.test.ts` 扩展**：
   - 在公开 `ToolEnabledAgent.run` seam 复现 `dd6b85b`：首次 patch 将 `value != NULL && value !== false` 放宽为 `value != NULL`，mutation 后完整复读仍包含 aria/data 注释和原 `popover` 三元式；
   - 实现前精确得到 `1 failed`：objective review 只发出 `3` 次请求便接受“aria false passes”完成声明，没有 correction；
   - 最小修复后得到 `1 passed`：共 `5` 次请求，objective review 发起唯一 correction，随后再次完整复读并进入 final review；修复条件为 `value !== false || name[4] == '-'`，`popover` 表达式原样保留；
   - 既有 objective input retry 场景新增合同持续性断言，不新增另一套恢复流程。

3. **效果**：
   - `aria-* false` 继续序列化，普通属性的 `false` 移除行为不再被扩大，`popover` 的既有 true 特例保持原样；
   - 提示材料、post-write 复读投影和 correction 输入构造三类问题已分开：前两者由 formal snapshot 证明充分，correction 未发生，当前修复仅针对真实发生的 objective semantic review 误判；
   - 技术债裁决：子集外行为误扩=`fix_now` 已完成本地闭环；真实模型外部有效性仍由新 identity Gate 与唯一 formal 独立验证。

##### 验证结果

- TypeScript 编译无错误：`corepack pnpm build:incremental` 与 `corepack pnpm verify:build` 均通过；
- 两个 owner 测试文件 `121/121` 通过（含 `1` 个新增子集行为回归）；同口径 Agent/structured-output 集合 `10` 个文件 `252/252` 通过；
- `corepack pnpm verify:coding-benchmark`、`corepack pnpm verify:coding-ci` 与 `git diff --check` 均通过；
- 关键行为验证确认 correction 只恢复目标子集条件，相邻 `popover` 三元表达式未进入 patch。

##### 后续计划

- **下一步准备做什么**：创建只包含本轮 Agent 源码、测试和本计划文档的本地提交，形成新 source identity；随后建立 detached clean harness，依次完成 frozen offline install、完整 build、独立 verifier、launcher/fixture 合同、Windows 零凭证 dry-run、敏感值/资源收敛和 formal prepare-only；
- **为什么先做它**：本地回归只能证明合同接线，固定新 identity 的干净 harness 才能排除主工作区未提交状态、旧 dist 或 fixture 漂移，并在不调用模型前验证正式输入；
- **当前还缺的关键闭环**：新 identity 的全部零模型 Gate，以及全部通过后唯一 Windows formal 的真实 mutation、objective correction、evaluator、合法终态、usage/cost、敏感值和零残留；未闭合前不启动 WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：`c124741` Windows 零模型 Gate（2026-08-19）

##### 已完成内容

1. **`c124741` detached clean harness 建立与构建**：
   - source/harness 固定为 `c1247417c805fb9ef3a95469eed93abab451498e`，Git tracked workspace clean；canonical lockfile/content SHA-256=`844c0021...` / `e281514e...`；
   - `corepack pnpm install --offline --frozen-lockfile` 解析 `493` 个包，downloaded=`0`，完整 build 与独立 `verify:build` 均通过；
   - Windows launcher、v3 fixture/evaluator 与额外 contract 合计 `36/36` 通过。

2. **Windows 零凭证 dry-run 完成**：
   - artifact=`artifacts/p0-web-subset-behavior-canary-c124741-preact-windows-dry-run-r1`，run=`real-web-ui-regression-windows-a1-1787088792610`，report SHA-256=`d3b1111b9a16f58b5ed93c45ef2f03072c55d9d3c3b8fa7064e4caaa0a9b0dbb`；
   - 双 preflight=`passed/passed`，credentials/model calls/usage=`false/0/not_reached`，events/trace/patch=`0/0/0 bytes`；
   - Gateway 端口/认证=`9,804/9,812ms`，停止耗时 `17ms`；harness/source identity 均为 clean、exact `c124741`。

3. **敏感值、资源与 formal prepare-only Gate 完成**：
   - clean harness、artifact、fixture 与 runtime 共扫描 `42,855` 个常规文件，跳过 `1,265` 个链接，unreadable/真实 Provider key/repository input 命中=`0/0/0`；
   - dry-run 新生成 `.env/.env.local` 经 containment、普通文件、无 reparse point 与 SHA-256 校验后送入 Windows 回收站；cleanup log 已记录，listener/相关 Node/剩余 env=`0/0/0`；
   - formal input SHA-256=`9f72c1b9c1a122772ee0de44c477b5e472ad33bccc7f85339a9e74677ebb21d5`，绑定 dry-run receipt SHA-256=`0e74aee3cc8cbff687f95b61aa9c5d0cbfcac79cbd50e049ec07831b0c7ebe6d`；prepare-only 最终为 Gateway/benchmark spawned=`false/false`；
   - formal 固定 `deepseek-v4-flash`、Provider retry=`0`、`12 turns / 24,000 tokens`、高峰价 `$0.0125/$0.375/$1.125 per 1M tokens` 与费用窗口 `$3.24792054 -> $3.34792054`；缺少显式 pricing 的首次 prepare 已在启动进程前失败关闭。

4. **效果**：
   - 当前 identity 已通过调用模型前的全部干净安装、构建、fixture、凭据隔离、双 preflight、readiness/auth、费用和敏感值 Gate；
   - dry-run 未产生模型事件或费用，formal 输入与 frozen repository snapshot 已精确绑定；
   - 只有一次 `c124741` Windows formal 被开放；任一真实结果都将立即冻结，不重跑、不预先启动 WSL2。

##### 验证结果

- TypeScript workspace 完整 build 与独立 `verify:build` 无错误；launcher/fixture/contract `36` 个测试全部通过（本 Gate 新增测试=`0`）；
- dry-run 双 preflight、零 usage、空 events/trace/patch、敏感扫描、env 回收站清理、资源收敛和 formal prepare-only 全绿；
- 模型调用=`0`、新增费用=`$0`，一般费用守卫仍为 `33.52104432 RMB < 50 RMB`，最坏累计守卫仍为 `47.62007744 RMB < 50 RMB`。

##### 后续计划

- **状态**：该 formal 已执行并冻结，详见下一条实现结论；不重跑、不启动其 WSL2。

#### P0 Web 代表实现结论：`c124741` Windows formal correction 输入失败（2026-08-19）

##### 已完成内容

1. **唯一 Windows formal 执行并冻结**：
   - artifact=`artifacts/p0-web-subset-behavior-canary-c124741-preact-windows-formal-r1`，run=`real-web-ui-regression-windows-a1-1787089300482`；source/harness 均 clean、exact `c1247417c805fb9ef3a95469eed93abab451498e`；
   - CLI=`exit 0 / run.completed`，benchmark=`failed / product_workflow`，tests/regression=`false/1`，patchAccepted/taskCompleted=`false/false`；
   - 模型固定 `deepseek-v4-flash`，Provider retry=`0`，model calls=`8/8`，input/output=`16,529/758`，provider cost=`$0.00468474`，duration=`11,837ms`。

2. **提示材料、复读证据投影与 correction 输入构造分层归因**：
   - 初始 mutation 将 `value != NULL && value !== false` 放宽为 `value != NULL`；post-write `file_read` 完整返回 `5,673` bytes，objective review 随后发出 correction；
   - correction 又将条件改回原始 `value != NULL && value !== false`，最终完整复读 `5,692` bytes 后 patch 为空、回归测试失败；该 correction 保留了普通 `false` 行为，却同时删除了所需的 `aria-* false` 正例行为；
   - task prompt、完整当前源码和 subset behavior preservation 合同均已进入 review 输入，因此排除 stale/incorrect context 与复读证据投影缺失；缺口收敛为 correction 输入构造只验证“子集外不扩大”，没有强制验证“目标正例仍满足”，导致 correction 抵消初始 mutation。

3. **终态、敏感值与资源审计**：
   - events/trace 连续且合同通过，唯一 `run.completed`，usage=`complete/provider_reported`；preflight/snapshot=`passed/passed`，Gateway readiness/auth=`10,622/10,630ms`，停止=`17ms`；
   - formal runtime 的 `.env/.env.local` 已完成 containment、常规文件、无 reparse point 与 SHA-256 校验后送 Windows 回收站并记录 cleanup log；扫描 `43,169` 个常规文件，unreadable/Provider key/repository input=`0/0/0`，listener/相关 Node/剩余 env=`0/0/0`。

4. **效果与技术债裁决**：
   - `c124741` formal 永久冻结，不重跑、不启动该 identity 的 WSL2；
   - correction 语义保持缺口=`fix_now`；不修改 evaluator、证据投影、Tool 状态机或执行预算，不把失败误判为 stale context。

##### 验证结果

- formal 的 `7` 个工具调用与 `8/8` model calls、usage/cost、terminal、patch 和冻结 evaluator 均已审计；
- evaluator 正确拒绝最终未保留 `aria-* false` 行为的 workspace（`changed_paths=0`、changes.patch 为空）；
- cleanup log 与资源收敛核验完成，新增费用=`$0.00468474`。

##### 后续计划

- **下一步准备做什么**：测试先行新增“correction 不能把失败的目标正例改回原始条件”的公开 Tool Agent 回归；随后强化 objective review、唯一 input retry 与 final review 的双侧合同，明确必须同时证明目标正例和子集外反例；
- **为什么先做它**：本次真实 formal 已证明 stale read 与 evidence projection 不是根因，最小修复应只补 correction 输入构造的正例保持约束，避免扩大为通用语义推断或修改 evaluator；
- **当前还缺的关键闭环**：失败测试、最小合同修复、owner/同口径回归、build/合同 Gate，以及新 identity 的唯一 Windows formal；未全绿不进入 WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：`fe49d51` Windows 零模型 Gate（2026-08-19）

##### 已完成内容

1. **`fe49d51` detached clean harness 建立与构建**：
   - source/harness 固定为 `fe49d51e3335706b58a2bbb3c27e7954a6f47444`，Git tracked workspace clean；
   - 完整 workspace build、独立 `verify:build`、`verify:coding-benchmark` 与 `verify:coding-ci` 均通过；
   - Windows launcher `17/17`、v3 fixture/evaluator `11/11`、benchmark contract `13/13`、verifier contract `6/6` 全部通过。

2. **Windows 零凭证 dry-run 完成**：
   - artifact=`tmp/p0-web-positive-witness-canary-fe49d51-windows-dry-run-r1-artifact`，run=`real-web-ui-regression-windows-a1-1787091094134`，端口=`28951`；
   - 双 preflight=`passed/passed`，credentials/model calls/usage=`false/0/not_reached`，events/trace/patch=`0/0/0 bytes`；
   - Gateway readiness/auth=`10,012/10,020ms`，listener/相关 Node=`0/0`，harness 构建后 clean。

3. **敏感值、资源与 formal prepare-only Gate 完成**：
   - dry-run 扫描 `8,059` 个常规文件，unreadable/真实 Provider key/repository-input blob 命中=`0/0/0`；自动生成的 `.env/.env.local` 已按持续授权送入 Windows 回收站；
   - formal repository input SHA-256=`36e10f162c741a51199568e61fb04f871927f435abd62f2740ba6f01e9ef28ee`；prepare-only 确认 Gateway/benchmark spawned=`false/false`，Provider key 未进入命令参数；
   - formal 固定 `deepseek-v4-flash`、Provider retry=`0`、`12 turns / 24,000 tokens`、高峰价 `$0.0125/$0.375/$1.125 per 1M tokens` 与费用窗口 `$3.25260528 -> $3.35260528`。

4. **效果**：
   - 当前 identity 已通过调用模型前的全部干净构建、fixture、凭据隔离、双 preflight、readiness/auth、费用和敏感值 Gate；
   - dry-run 未产生模型事件或费用，formal 输入与 frozen repository snapshot 已精确绑定；
   - 只开放一次 `fe49d51` Windows formal；该 run 执行后永久冻结，不重跑、不启动同 identity 的 WSL2。

##### 验证结果

- TypeScript workspace 完整 build 与独立 `verify:build` 无错误；launcher/fixture/contract `47` 个测试全部通过（本 Gate 新增测试=`0`）；
- dry-run 双 preflight、零 usage、空 events/trace/patch、敏感扫描、env 回收站清理、资源收敛和 formal prepare-only 全绿；
- 模型调用=`0`、新增费用=`$0`，费用守卫保持 `33.55852224 RMB < 50 RMB`。

##### 后续计划

- **状态**：该 formal 已执行并冻结，详见下一条实现结论；不重跑、不启动其 WSL2。

#### P0 Web 代表实现结论：`fe49d51` Windows formal 非最小 correction 失败（2026-08-19）

##### 已完成内容

1. **唯一 Windows formal 执行并冻结**：
   - artifact=`artifacts/p0-web-positive-witness-canary-fe49d51-preact-windows-formal-r1`，run=`real-web-ui-regression-windows-a1-1787091415408`；source/harness 均 clean、exact `fe49d51e3335706b58a2bbb3c27e7954a6f47444`；
   - CLI=`exit 4 / run.failed`，benchmark=`failed / product_workflow`，tests/regression=`true/0`，patchAccepted/taskCompleted=`false/false`；
   - 模型固定 `deepseek-v4-flash`，Provider retry=`0`，model calls=`6/6`，input/output=`13,819/807`，provider cost=`$0.00428042`，duration=`10,030ms`。

2. **提示材料、复读证据投影与 correction 输入构造分层归因**：
   - 初始 patch 用 `name[0] == 'a' && ... && name[4] == '-'` 恢复 `aria-* false`，测试通过但不满足冻结 evaluator 要求的最小条件 `name[4] == '-'`；
   - objective input retry snapshot 同时包含原始 `smallest change` 任务、mutation 后完整当前条件、正例/子集外行为合同和唯一 required path，因此排除 stale/incorrect context、任务提示缺失与复读证据投影缺失；
   - correction 仍只重复当前源码，执行前 redundant Gate 正确拦截并失败关闭；缺口收敛为 input retry 未把“当前 predicate 已过度展开”明确识别为仍需收缩的 task-relevant semantic delta。

3. **终态、敏感值与资源审计**：
   - patch bytes/SHA-256=`622/6d4435814f74d48126f423a31dbcf20b402c0a7eddddb9a76fc5ac76c65494a0`，`result.json=null`，最终错误为 correction 只重复 current-source block；
   - 双 preflight=`passed/passed`，Gateway 首 stdout/端口/认证=`2,030/10,615/10,623ms`，stderr=`0 bytes`，child 完整退出；
   - clean harness、artifact、fixture 与 runtime 扫描 `43,169` 个常规文件，跳过 `1,265` 个链接，unreadable/Provider key/repository-input blob 命中=`0/0/0`；
   - formal runtime 的 `.env/.env.local` 经 containment、普通文件、无 reparse point 与 SHA-256 校验后按持续授权送入 Windows 回收站；cleanup log=`tmp/p0-web-positive-witness-canary-fe49d51-preact-windows-formal-r1-sensitive-cleanup.log`，SHA-256=`140ddd5e26a33959f866a578c314cd73213df33ab36f57305cf31b9bd450fc04`，listener/相关 Node/剩余 env=`0/0/0`。

4. **效果与技术债裁决**：
   - `fe49d51` formal 永久冻结，不重跑、不启动该 identity 的 WSL2；
   - redundant Gate、正反 witness、完整当前源码和费用/执行预算均保持闭合；过度展开 predicate 未收缩=`fix_now`，只修改 objective input retry 输入合同，不放宽 evaluator 或增加通用语义猜测。

##### 验证结果

- formal 的 `5` 个工具调用、`6/6` model calls、usage/cost、失败终态、patch 和冻结 evaluator 均已审计；
- evaluator 正确拒绝测试通过但表达式非最小的 workspace，redundant Gate 正确阻止第二个 no-op correction 进入 executor；
- cleanup log、真实敏感值扫描与资源收敛核验完成，新增费用=`$0.00428042`。

##### 后续计划

- **下一步准备做什么**：在公开 Tool Agent seam 测试先行复现“初始 predicate 过度展开、首次 correction 冗余、唯一 input retry 应收缩为最小 semantic delta”，再只收紧该 retry 的输入合同；
- **为什么先做它**：formal 已证明任务材料和当前源码完整可达，且 redundant Gate 工作正确；修补唯一发生缺口比改写 evidence projection、evaluator 或状态机更窄；
- **当前还缺的关键闭环**：red/green、owner/Agent 全量回归、build/合同 Gate，以及新 identity 的全部零模型 Gate与唯一 Windows formal；未全绿不进入 WSL2、完整矩阵、candidate v4 或 P2-C。

#### P0 Web 代表实现结论：objective input retry 最小 semantic delta（2026-08-19）

##### 已完成内容

1. **`react-workspace-mutation.ts` 修改**：
   - objective input retry 在任务明确要求最小变更时，将当前过度具体或过度展开的 predicate 视为仍未完成的 task-relevant behavior；
   - 要求由任务中的正例和子集外反例证明最小充分条件，只替换该 predicate，并保持其余当前源码不变；
   - 未修改 objective review/final review 状态机、证据投影、redundant Gate、evaluator、turn/token、Provider retry 或费用上限。

2. **`tool-agent-workspace-mutation.test.ts` 扩展**：
   - 新增公开 `ToolEnabledAgent.run` 同形回归，覆盖初始字符枚举 patch、完整 post-write 复读、首次 redundant correction、唯一 input retry、最小 correction、再次完整复读和 final review；
   - 修复前同一测试两次稳定得到 `1 failed`，均在 `4` 次模型请求后以 current-source block 失败关闭；
   - 修复后执行链为 `6` 次模型请求、`2` 次真实 mutation，第二次只把展开条件收缩为 `value !== false || name[4] == '-'`，失败 correction 从未进入 executor。

3. **效果**：
   - task 的 `smallest change` 材料、mutation 后当前 predicate 和 input retry 构造分别有独立断言，排除旧源码混入和投影假绿；
   - `aria-* false` 正例与普通 `false` 反例行为同时保留，已正确的相邻源码不进入 correction；
   - 技术债裁决：过度展开 predicate 未收缩=`fix_now` 已完成本地闭环；真实模型外部有效性仍由新 identity Gate 和唯一 formal 独立验证。

##### 验证结果

- TypeScript 编译无错误：`corepack pnpm build:incremental` 与 `corepack pnpm verify:build` 均通过；
- 两个 owner 测试文件 `123/123` 通过（含 `1` 个新增 semantic-delta 回归）；Agent 包 `57` 个文件、`672` 个测试通过，另有 `1` 个真实 Provider probe 按既有条件跳过；
- `corepack pnpm verify:coding-benchmark`、`corepack pnpm verify:coding-ci` 与 `git diff --check` 均通过；本实现环节模型调用=`0`、新增 Provider 费用=`$0`；
- 轻量对抗性 review 确认简化必须由任务正例/反例证明，未引入无证据的通用 predicate 重写。

##### 后续计划

- **下一步准备做什么**：提交仅包含本轮 Agent 源码、测试和计划文档的本地改动形成新 identity；随后建立 detached clean harness，完成 frozen offline install、完整 build、独立 verifier、launcher/fixture/contract、Windows 零凭证 dry-run、敏感值/资源收敛和 formal prepare-only；
- **为什么先做它**：本地回归已证明输入合同接线，但固定新 identity 的干净 harness 才能排除主工作区用户改动、旧 dist、fixture 或 repository input 漂移；
- **当前还缺的关键闭环**：新 identity 的全部零模型 Gate，以及全部通过后唯一 Windows formal 的最小 patch、冻结 evaluator、合法终态、usage/cost、敏感值和零残留；未闭合前不启动 WSL2、完整矩阵、candidate v4 或 P2-C。

### 6.6 费用与禁止范围

当前授权窗口：

- observed conservative upper=`$2.45688570`；
- reserved=`$0.94221000`；
- unobservable reserve=`$0.80000000`；
- 守卫上界=`33.59276560 RMB < 50 RMB`。
...
`a72f127` 唯一 Windows formal 已执行、失败并冻结；产品 mutation 成功，但 terminal/report usage 因 CLI `read ENOTCONN` 不可观测，完整 `$0.10` 已计入预留。DeepSeek 新价格自 `2026-08-17 00:00` 生效，生效后 `36` 个历史可观测 formal 已统一按高峰价和输入全 miss 重算，差额 `$0.12570178` 已加入保守 observed 上界。`f0615b8`、`9a7c3b3`、`887bcd7`、`de931cc` Windows/WSL2、`5200317` Windows/WSL2、`0cd7d13` Windows/WSL2、`2977780` Windows/WSL2、`d6d7367`、`d01030a`、`8cee589`、`09b5498`、`cb01ccd`、`abe40b1`、`dd6b85b`、`c124741` 与 `fe49d51` Windows formal 的 provider-reported cost=`$0.00358616/$0.00302790/$0.00235180/$0.00316938/$0.00334516/$0.00291315/$0.00278265/$0.00639158/$0.00244161/$0.00635007/$0.00606781/$0.00690650/$0.00292393/$0.00271056/$0.00353063/$0.00362468/$0.00297676/$0.00468474/$0.00428042` 均已加入 observed。`8a67630` 与 `2e51cb9` Windows formal 均在 benchmark/model spawn 前以 infrastructure failure 冻结，model calls=`0`、仓库本地新增费用=`$0`，不改变 observed。Stage 0D 累计池仍为 `$5.00`，最坏累计池加 reserved 守卫=`47.69179872 RMB < 50 RMB`；`2977780` 双平台以及 `d6d7367`、`d01030a`、`8cee589`、`09b5498`、`cb01ccd`、`abe40b1`、`dd6b85b`、`c124741`、`fe49d51` Windows formal 均已冻结，项目记录不能替代 Provider 外部账单。
...
- 重跑 `09b5498` 已执行的 Windows dry-run/formal，或为该失败 identity 启动 WSL2；
- 重跑 `cb01ccd` 已执行的 Windows dry-run/formal，或为该失败 identity 启动 WSL2；
- 重跑 `abe40b1` 已执行的 Windows dry-run/formal，或为该失败 identity 启动 WSL2；
- 重跑 `dd6b85b` 已执行的 Windows dry-run/formal，或为该失败 identity 启动 WSL2；
- 重跑 `c124741` 已执行的 Windows dry-run/formal，或为该失败 identity 启动 WSL2；
- 重跑 `fe49d51` 已执行的 Windows dry-run/formal，或为该失败 identity 启动 WSL2；
...
| objective input correction 连续使用 stale/incorrect context | `fix_now` | `cb01ccd` 已将 correction 证据收敛为每个 required path 最新完整复读；formal 精确重放进一步证明首次 review 与 retry 均包含 mutation 后当前条件，排除提示陈旧、旧 read 混入和首次投影漏条件，既有 current-source 修复保持闭合 |
| objective correction 只重复当前源码并误判完成 | `fix_now` | formal 捕获“重复已有注释 + 删除后原样加回未变化行”仍进入 executor；现以最新完整 required-path 源码做窄形状执行前判定，首次命中复用唯一 input retry、第二次立即失败关闭；`abe40b1` formal 未再执行冗余 correction，证明该 Gate 真实生效 |
| objective correction 修复语义但等价重写相邻代码 | `fix_now` | `abe40b1` correction 恢复 aria 行为且测试通过，但展开已正确的 `popover` 三元表达式，冻结 evaluator 正确拒绝；`dd6b85b` 已完成最小变更合同及本地 Gate，但 formal 没有触发 correction，故只保留“本地闭合、外部未覆盖”结论；未放宽 evaluator 或引入通用语义猜测 |
| 首次 mutation 扩大子集外行为且 objective review 误判完成 | `fix_now` | `dd6b85b` 唯一 formal 将 aria 正例泛化到全部普通 `false`，完整 post-write 复读后仍直接宣称完成；已排除 stale context、证据投影和 correction 输入构造，并在公开 Tool Agent seam 测试先行接入子集行为保持合同，owner `121/121`、同口径 `252/252` 和 build/合同 Gate 全绿，待新 identity 外部验证 |
| objective input retry 未收缩过度展开的当前 predicate | `fix_now` | `fe49d51` 唯一 formal 的任务提示与最新完整源码均正确，redundant Gate 也正确阻止 no-op correction；现已在公开 Tool Agent seam 测试先行要求由任务正/反 witness 证明最小 semantic delta，仅收缩 task-relevant predicate；owner `123/123`、Agent 全量 `672/672` 和 build/合同 Gate 全绿，待新 identity 外部验证 |
| 冗余 correction 后 final-review 投影遗漏关键条件 | `record_only` | prompt snapshot 证明重复注释落盘后条件被挤出最终 taskRelevantContexts；该状态已由上游执行前 Gate 阻断，本轮不为单一 downstream 症状扩大通用投影算法；若未来非冗余 correction 再现则独立拆分 |
| detached worktree 首次 pnpm offline 链接超时 | `record_only` | `09b5498` 与 `cb01ccd` clean harness 首次安装分别触达 `180s/300s` 工具上限；两次均在无残留进程、Git clean、依赖目录已建立后由同一 frozen offline 命令幂等续跑完成，downloaded=`0`，随后 build、verify 与 `28/28` 测试全绿；归类为 NTFS 本地链接耗时，不外推为产品或锁文件失败 |
...
- `a72f127`、`f0615b8`、`9a7c3b3`、`887bcd7`、`de931cc`、`5200317`、`0cd7d13`、`2e51cb9`、`d6d7367`、`d01030a`、`8cee589`、`09b5498`、`cb01ccd`、`abe40b1`、`dd6b85b`、`c124741` 和 `fe49d51` 的失败证据均保持冻结；后继 `2977780` 已关闭 required-mutation 双平台代表 canary。当前离线重算已将全部 `37` 个历史失败稳定分类为 `30/5/2`，`unknown=0`；最新 `fe49d51` 证明任务提示、当前源码复读和 redundant Gate 均正确，真实缺口是唯一 objective input retry 没有把已过度展开的 task-relevant predicate 收缩为任务要求的最小充分条件。
...
DeepSeek 调价后，生效后 `36` 个历史 formal 已按高峰价保守重算，`f0615b8`、`9a7c3b3`、`887bcd7`、`de931cc` Windows/WSL2、`5200317` Windows/WSL2、`0cd7d13` Windows/WSL2、`2977780` Windows/WSL2、`d6d7367`、`d01030a`、`8cee589`、`09b5498`、`cb01ccd`、`abe40b1`、`dd6b85b`、`c124741` 与 `fe49d51` Windows formal 的 provider-reported cost 均已入账；observed conservative upper=`$2.45688570`，当前费用守卫为 **33.59276560 元人民币**，低于 **50 元人民币**授权上限。`a72f127` terminal/report usage 不可观测，仍保守预留完整 `$0.10`；runner 累计池保持 `$5.00`，加现有 reserved 后的最坏守卫为 **47.69179872 元人民币**。`8a67630` 与 `2e51cb9` Windows formal 均未启动 benchmark/model、仓库本地新增费用=`$0`；上述已执行 formal 均永久冻结，未提高费用、turn/token 或 retry，外部服务商账单仍需单独核对。

当前不会重跑已冻结版本，不会提高模型预算，不会启动完整付费矩阵，不会 push、公开发布或执行生产操作。

### 9.7 后续计划

- **下一步准备做什么**：具体状态以文末唯一进度表为准；先提交 objective input retry 最小 semantic-delta 合同的测试先行修复，再形成新 identity 完成 detached Windows 全部零模型 Gate。
- **为什么先做它**：本地 red/green、owner/同口径回归、build 与 benchmark/CI 合同已闭合；干净 source/harness identity 是再次开放唯一付费 formal 的最后无费用前置条件。
- **当前还缺的关键闭环**：新 identity 的 build/fixture/dry-run/双 preflight/readiness/auth/prepare-only，以及其后唯一 formal 的 correction、evaluator、合法终态、usage/cost、敏感值与零残留；未全绿不进入 WSL2、完整矩阵、candidate v4 或 P2-C。
...
| P0：Benchmark v3 与外部有效性 | P0 | **`fe49d51` formal 非最小 correction 失败并冻结；semantic-delta 合同本地完成** | formal 初始 patch 以字符枚举恢复 aria 正例但不满足最小表达式，objective retry 又复读当前源码；任务提示、最新完整复读与 redundant Gate 均正确。现公开 Tool Agent seam 已 red/green，owner `123/123`、Agent 全量 `672/672`、build/benchmark/CI 合同全绿 | 新 identity 零模型 Gate + 唯一 formal 约 0.5 人日 | 先提交新 identity 并完成 detached Windows 全部零模型 Gate；费用窗口从 `$3.25688570 -> $3.35688570`，只在 Gate 全绿后评估唯一 Windows formal；禁止重跑 `fe49d51`/`c124741`/`dd6b85b` 等冻结 identity，未全绿不启动 WSL2 |
