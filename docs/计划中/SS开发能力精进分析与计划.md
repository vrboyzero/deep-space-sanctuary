...
5. **效果**：
   ...
   - corrected analysis 的 `accepted_patch_regression=1/1` 当前本地根因路径闭合，旧 run 仍保持失败，不重解释为历史通过。
