...
- `packages/belldandy-core/src/user-worktree-runtime.ts`: 用户级受管 worktree 的持久化 owner、受控 create、status、worktree-base diff 与本地交付 preview；create 仅使用稳定 conversation/run 派生的受管 ID，源仓/登记失败时只回收未变化的刚建 worktree，重试幂等；diff 复用 immutable snapshot/hunk artifact 并校验记录的 base/path，status 展示 base/current commit、branch、dirty/冲突/额外提交与保留原因；`apply/remove/stage/commit/branch` 均先执行无写入 Git gate，再签发绑定 worktree/base/current/target/patch/index tree/message/identity/branch 的短期 receipt，独立 confirm 消费 receipt 后重跑 final gate；stage 只处理既有 tracked 变更，commit 拒绝可改写结果的本地 hook，branch 只以非 force 方式新建指向已确认 commit 的本地 ref，成功后复核 postcondition 并写本地审计；失败保留无内容泄露 evidence，不提供 push、PR 或远端写入
- `packages/belldandy-core/src/remote-delivery-runtime.ts`: 阶段 7 远端 Git 单 owner；严格解析 exact remote URL/branch/PR base allowlist，拒绝内嵌凭据、重复 binding 与 GitHub URL/repository 漂移；preview 签发 5 分钟一次性 receipt，绑定 real repo/cwd、source branch/HEAD/upstream、remote URL hash、目标 OID 与 diff hash，confirm 先写脱敏 pending audit 再复核全部 binding；push 只执行非 force exact commit-to-ref refspec 并以 `ls-remote` 验证 postcondition，PR body 只通过 stdin 交给有 deadline 的 `gh` adapter，receipt/audit 仅保存内容 hash；不支持 merge、tag、release、部署或仓库保护绕过
...
- `packages/belldandy-core/src/extension-runtime-supervisor.ts` / `extension-runtime-contract.ts`: Host API v2 grant、Tool/Hook/Skill owner 原子发布、generation/deadline、版本化有界 NDJSON contract、fatal revoke 与逆序 shutdown owner
...
- `packages/belldandy-core/src/extension-runtime-lease.ts`: `extensionId + contentSha256` 持久 runtime lease、Gateway 重启 stale cleanup 和 disable/update/uninstall 离线变更 Gate
