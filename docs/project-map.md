...
- `packages/belldandy-skills/src/code-intel/code-intel.ts`: Provider capability 选择、deadline/abort、结果上限、opaque cursor、workspace containment、external allowlist 与稳定错误归一化 owner
...
- `packages/belldandy-skills/src/command-sandbox.ts`: coding run 命令的 OCI（Docker/Podman）sandbox 配置、digest-pinned image、runtime probe、无 Shell invocation、Unix/WSL 宿主 UID/GID 的最小 `--user` 投影和短生命周期 env-file owner；目标平台后端不可用时返回可审计的 fail-closed 拒绝，禁止回退到宿主命令执行或自动拉取镜像
