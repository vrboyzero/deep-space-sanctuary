...
- `packages/belldandy-core/src/user-worktree-runtime.ts`: 用户级受管 worktree 的持久化 owner、受控 create、status、worktree-base diff 与本地交付 preview；create 仅使用稳定 conversation/run 派生的受管 ID，源仓/登记失败时只回收未变化的刚建 worktree，重试幂等；diff 复用 immutable snapshot/hunk artifact 并校验记录的 base/path，status 展示 base/current commit、branch、dirty/冲突/额外提交与保留原因；`apply/remove/stage/commit/branch` 均先执行无写入 Git gate，再签发绑定 worktree/base/current/target/patch/index tree/message/identity/branch 的短期 receipt，独立 confirm 消费 receipt 后重跑 final gate；mutation 前独占写 receipt 级 `started` audit，完成后原子替换为 `succeeded/uncertain`，started audit `ENOSPC` 返回 `audit_unavailable` 且不启动 mutation，completion `ENOSPC` 保留 applied/uncertain 与 canonical started evidence；重启后从 audit 或 receipt lock 失败关闭且不自动重放；stage 的 canonical started audit 只在 authoritative worktree 的 HEAD/branch、cached patch hash、新 index tree、mode 与零 unstaged tracked drift 全部匹配 receipt 时补写 succeeded，并清理同 receipt 遗留 audit temp，恢复测试以 Git Trace 验证不再次执行 `git add`；stage 只处理既有 tracked 变更，commit 拒绝可改写结果的本地 hook，branch 只以非 force 方式新建指向已确认 commit 的本地 ref；失败保留无内容泄露 evidence，不提供 push、PR 或远端写入
