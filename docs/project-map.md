...
- `packages/belldandy-core/src/cli/commands/coding-run/stdio.ts`: `bdd coding-run stdio` 进程入口；stdin/stdout 只承载 NDJSON，通过 Gateway 转发受限 Conversation 请求、已验证 control、单个 cursor 事件订阅与只读 revision artifact query，不直接拥有领域运行时
...
- `packages/belldandy-skills/src/code-intel/lsp-process-host.ts`: language-neutral out-of-process LSP Host seam；集中 stdio JSON-RPC framing、initialize/initialized、受 profile 白名单约束的 workspace/configuration/workspaceFolders、多 folder 初始化、client notification 与动态 capability registration/work-done progress token 状态/有界静默等待、deadline/cancel、显式环境、bounded stderr、decoded response 字节硬限、单并发 Gate、shutdown、kill/reap 与生命周期 diagnostics；Host 本身不直接决定 OCI admission 或 production eligibility
