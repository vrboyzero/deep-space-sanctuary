...
- `scripts/aggregate-coding-agent-benchmark.mjs`: v1/v2/v3 基线 evidence 聚合与离线重算校验 owner；按所选冻结 manifest 拒绝覆盖、跨 revision、样本重复、source identity 漂移和缺失 artifact，v2/v3 额外绑定单一 harness identity，v3 按 144 项矩阵保留 B/C 专属 artifact 与 browser PNG，并把 source report 与声明文件汇成 partial 或 completed baseline
- `scripts/run-coding-agent-benchmark.mjs`: 阶段 0B/0C/0D 与 v3 Windows native runner；严格核对实际平台指纹和显式 task 白名单，串联 fixture、既有 Coding CI Headless 链与 partial benchmark report；导航 shadow candidate 继续只允许 v3 `real-js.bug-fix`，CodeIntel uplift candidate 只允许冻结的四个 TS/JS cohort task 且保持原 prompt；v3 WSL run 保持 Linux workspace 归 fixture/Git/evaluator，并从显式 Gateway fixture root 派生同 run 的 Windows/UNC workspace，仅交给远端 Coding CI cwd；v3 CLI 装配 native system harness，在创建运行目录前校验版本化 repository config/receipt 或精确 system capability，并按 Provider 类型保存 snapshot preflight/receipt 或 scenario/evidence，专属 JSON 限制 1 MiB 且拒绝 credential 字段；browser evidence 落盘前复核 `browser-screenshot.png` 的常规文件、5 MiB 上限与 SHA-256，并以 `systemBrowserScreenshot` 纳入 report；仅当最后规范化 usage 为 Provider 上报且终态 completeness 为 `complete` 时接受费用，真实凭据 run 按剩余额度透传 `max-cost-usd`，usage/cost 不可观测时停止后续 task；不执行远端 Git 写入
- `scripts/run-coding-agent-benchmark-wsl.mjs`: v1/v2/v3 Windows host launcher；使用目标发行版的 `wslpath` 和无 shell 的 `wsl.exe --exec` 参数数组启动 Linux runner，转换 Linux fixture 路径时以 `--gateway-fixture-root` 保留原 Windows/UNC 根，供 runner 派生 Windows-host Gateway 可见 workspace；v3 额外转换 repository config 文件路径并保持内容由 Linux runner 解析，透传分批续跑的 prior cost 与三项非敏感 Provider pricing，token auth 只通过 child env/`WSLENV` 转交，不进入参数或 artifact；不准备或跨平台修复依赖缓存
...
- `packages/belldandy-core/src/coding-run/output-schema.ts`: Headless structured output 的 Core owner；限制 schema 为 1 MiB、编译 JSON Schema，并只接受严格完整 JSON 或单一显式 JSON code block；供 Gateway 可信 Agent 注入与 CLI 防御性终态校验共用
...
- `scripts/run-code-intel-truth-set.mjs`: 从实际 `dist` 执行固定 symbol/definition/reference/implementation truth set，核对 source/runtime identity、逐 case 精确位置与 micro precision/recall，并以不可覆盖方式写出 Windows/WSL2 离线报告
...
- `scripts/run-code-intel-resource-soak.mjs`: 从同一 `dist` 在 Windows/WSL2 创建并清理临时 TS workspace，验证 session reuse/LRU、跨 revision cursor 拒绝、revision reload、dispose、raw/live heap、时延和零残留，并提供双端 identity 比较；不访问 Gateway、模型、付费 Provider、网络、凭据或生产 workspace
