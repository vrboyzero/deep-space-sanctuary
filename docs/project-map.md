...
- `scripts/coding-agent-process-restart-harness.mjs`: 阶段 0C 受控 loopback Gateway 子进程、首个已接受 run 的 PID 级重启、旧 binding 的订阅/取消探测与收敛 artifact owner；重启前经 coding-run stdio 固化 TaskProjection epoch/revision cursor，重启后要求旧 cursor 以 `cursor_stale` 失败关闭；本地受控 Gateway 始终使用当前 OS 的 evaluator workspace，v2/Linux 冷加载 Gateway 与 stdio/CLI probe 使用 60 秒单操作上限，Windows/v1 保持 15 秒，并继续受任务总预算约束；不复用用户 Gateway 或环境配置
